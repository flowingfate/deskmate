...
用户只管理普通 Agents：在 Basic 编辑 description，在 Delegation tab选择允许委派的其它 Agents。移除独立 Sub-Agent 产品入口；其 renderer/CRUD/persist 源码在生产引用归零后整体删除，不留参考副本。
...
4. 不打开旧 AgentSubAgentsTab、独立 CRUD UI 或旧测试；交互与结构只沿用当前 Agent editor、现有设计系统和已确认产品契约；
...
生产引用清零后，整体删除对应旧组件、atom/hook、CRUD IPC/preload/shared channel，以及不再被任何代码使用的旧 persist SubAgents store/Markdown serializer；禁止为了继续编译而修改这些旧文件。磁盘上的旧数据目录不碰。

## 7. 不做

- 不做 runtime tool card/Dialog；
- 不改 manager；
- 不删除用户磁盘上的旧 Sub-Agent 数据；
- 不做浏览器/E2E/手工 smoke；
- 不新增/运行新单测。
...
- 搜索 production route/nav/provider不再使用旧 Sub-Agent入口，并确认计划删除的旧 UI/CRUD/persist 源码路径已消失；
- 检查 component行数；
...
Progress记录最终 tab key/route、patch字段、candidate/dangling datasource、已删除的旧 UI/CRUD/persist 源码闭包；Step13只做全局残留证明与文档收口；unit-test.md更新 editor data candidates。
