...
Step 4 已具备输入（2026-07-16）：`ToolContext` 的 `agentId/sessionId` 固定为 parent session identity；新 session 构造 `{ mode:'delegate', agentId:parentAgentId, sessionId:parentSessionId, delegateId:delegateAgentId }`。转换与 AppCommand dispatcher 已保留 discriminant，不得重新引入 `isSubAgent` 或 optional delegate。
...
6. ToolContext 使用 `mode:'delegate'`，agent/session 指向 parent，delegateId 指向执行 Agent；
