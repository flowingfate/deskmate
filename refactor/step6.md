...
Step 4 已具备输入（2026-07-16）：parent session lookup 固定为 `Profile.getAgent(agentId) → Agent.findSessionAcrossKinds(sessionId)`；delegate identity 不参与 parent store 定位。ToolContext 不传绝对路径，Step 6 直接在该 parent Session 实例上增加最小 subrun store API。
