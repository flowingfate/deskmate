...
function createTestUserDataDir(): string {
  const dirName = `deskmate-e2e-mock-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
 ...
 * Electron may have background tasks (startup update, etc.) that
...
function getElectronEntryPath(): string {
  const distMain = path.resolve(__dirname, '../../../out/main');
...
export const mockedChatReadyTest = base.extend<MockedChatReadyFixtures>({
  ...
  chatWindow: async ({ chatApp, preseededAuthData }, use) => {
    ...
    // Wait for React to mount first ("Starting DESKMATE" disappears)
    console.log('[E2E Mock Chat] Waiting for React to mount...');
    try {
      await window.waitForFunction(
        () => {
          const body = document.querySelector('body');
          // "Starting DESKMATE" is the pre-React loading text in index.html
    ...
    // Full chain: chat-textarea visible → selectPrimaryAgentOnStartup → startNewChatFor IPC → mock returns
    //
