...
function createTestUserDataDir(): string {
  const dirName = `deskmate-e2e-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
 ...
 * reads the root-level package.json (whose "main" field points to out/main/bootstrap.js),
 * which correctly resolves APIs that depend on package.json such as app.getVersion().
 *
 * 健康检查仍校验 main.js 存在 — bootstrap 会 require 它，缺失即 build 失败。
 */
function getElectronEntryPath(): string {
  const projectRoot = path.resolve(__dirname, '../../..');
  const mainJs = path.join(projectRoot, 'out/main/main.js');
  ...
  electronApp: async ({ testUserDataDir }, use) => {
    ...
    const app = await electron.launch({
      ...
      env: {
        ...process.env,
        NODE_ENV: 'test',
        // Specify the full userData path directly, bypassing compile-time substitution.
        // bootstrap.ts reads this value at runtime via process['env']['DESKMATE_TEST_USER_DATA_PATH'],
        // which takes priority over the USER_DATA_NAME injected by Vite `define`.
        DESKMATE_TEST_USER_DATA_PATH: testUserDataDir,
      },
