...
| Agent 委派运行时（pi/subagent，建设中） | `src/main/pi/subagent/` + `src/shared/types/subAgentRunTypes.ts` | 已有共享契约、未注册 cmdline facade 与 `lib/delegateExecutionScope.ts` 驱动的 delegate-only 能力边界；manager/session/persist 尚未接线，旧 `lib/subAgent` 仍是生产路径 | [ai.prompt.md](../src/main/pi/subagent/ai.prompt.md) |
