...
| Agent 委派运行时（pi/subagent，建设中） | `src/main/pi/subagent/` + `src/shared/persist/types/subrun.ts` | 已有持久化 run contract、parent-scoped subrun persist store、delegated-only `submit_result`/formal result reducer、未注册 cmdline facade 与 `lib/delegateExecutionScope.ts` 驱动的能力边界；manager/session 尚未接线，旧 `lib/subAgent` 仍是生产路径 | [ai.prompt.md](../src/main/pi/subagent/ai.prompt.md) |
