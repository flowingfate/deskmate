...
共 30+ 个命名空间，覆盖所有 IPC 通道：app、window、auth、signin、featureFlags、misc（folder/debug）、profile、agentChat、chatSession、models、llm、fs、workspace、mcp/mcpAuth、navigate、skills、builtinTools、subAgent、runtime、sync、update、msalAuth、sharepoint、logViewer（dev-only）。
...
| `persist:agent:updated` | `{profileId, agentId, record, detail}` | `Agent.persist()`（按 agentId 防抖）；`detail` 与 `record` 同时下推，省 renderer 一次 `getAgentDetail` 回查 |
...
agents.atom             ← agent:registry:updated[kind=agents] / agent:updated / agent:removed
                         （只持 `AgentRecord`，hot list 字段；含 primaryAgentId）
agentDetail.atom        ← agent:updated（拿 payload.detail 刷 cache） / agent:removed
                         （cold 字段 `AgentDetail`：systemPrompt / mcpServers / skills / ...；按 agentId
