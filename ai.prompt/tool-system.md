...
<!-- Last verified: 2026-07-18 (AppCmdContext runtime Profile propagation) -->
## 1. 范围
...
- `src/main/pi/subagent/commands/` —— 顶层 `subagent` 的 registry、grammar 与 production manager runner
...
> **现状**：普通 Agent 顶层工具为 **9**（`read/write/find/search/shell/ask/app/web/subagent`）。delegate context 下 catalog 排除交互式 `ask` 与真实 `subagent` 对象；其它工具保持普通能力，只有 `web research` 与已知 shell device-auth 在执行边界拒绝。
...
| **AppCmdContext 是 ToolContext 精确子集 + stdio helpers** | 显式收窄,不 spread；`ctx.profile` 是 runtime owner，命令不得回读 Registry 默认候选；任何字段漂移都走类型系统强制声明。 |
...
├── executeCommandFacade.ts        command + cmdline + ToolContext → parse / dispatch / format 共享执行 helper
...
│   │   ├── schedule/                   feature-gated 命令
│   │   └── ...
│   └── web/                       `web` 域 —— webCommands + 成员(与 app/ 逐字对等)
│       ├── index.ts               export const webCommands = new AppCommandRegistry() + eager 注册
│       ├── search.ts  image.ts    成员命令(各自导出 xCommand)
│       ├── fetch.ts   download.ts  (download 是唯一产出型命令)
│       ├── research.ts            human-in-the-loop web research 降级命令
│       └── kernel/                业务内核(bingWebSearch / fetchWebContent / download / research)
├── vendor/
│   ├── argsTokenizer.ts           Vendored from args-tokenizer 0.3.0 + [M1] 改动
│   └── argsTokenizer.LICENSE      上游 MIT 全文,逐字
└── __tests__/                     parseCmdline / flags / dispatcher + app/<域>/* + web/* 单测

src/main/pi/tools/
├── app.ts                         显式 LocalTool schema/description/handler + makeRouterCommand({ registry: appCommands })
├── web.ts                         显式 LocalTool schema/description/handler + makeRouterCommand({ registry: webCommands })
└── subagent.ts                    独立顶层 LocalTool；Profile command cache + executeCommandFacade
...
AppCmdContext 保持 parent identity 与 owning runtime `profile`。AppCommand 及其 kernel 直接接收 `ctx.profile` 或 `ctx.profile.store`；`profileId` 只用于 trace / URI / 只接收 ID 的外部 API，禁止用它在命令链中反查 Registry。normal execution 没有 delegate context；delegated run 仅拒绝会接管当前会话用户操作的 `web research`，其余 app/web 命令、下载与 MCP OAuth 保持普通行为。
...
| 测试成本 | 直接喂 typed object 测 handler | 要测"cmdline → tokenize → flags → run"整条链 | fixture 范式收敛在 `__tests__/app/mcp/_fixture.ts`(`vi.hoisted` 形态),后续域照搬;一次性成本不是持续成本 |
...
所有 LocalTool → AppCommand 迁移已完成；当前顶层 LLM-visible 工具为 **9**（`read` / `write` / `find` / `search` / `shell` / `ask` / `app` / `web` / `subagent`）。
其中 `app` 与 `web` 是两个**完全对等**的 router facade：二者都在自己的 LocalTool 文件内显式定义 schema、description 与 handler，并各自持有 `makeRouterCommand({ name, synopsis, registry })` 结果，路由自己的注册表（`appcmd/builtins/app/index.ts` 的 `appCommands` / `appcmd/builtins/web/index.ts` 的 `webCommands`）。三者复用 `executeCommandFacade()` 的 parse / dispatch / format；`subagent` 拥有独立 `pi/subagent/commands` registry 与 production manager runner，不属于 `app` 域。
...
| `subagent`（顶层） | list / describe / run |

实现仓:`app` 域成员在 `appcmd/builtins/app/<domain>/`，`web` 域成员在 `appcmd/builtins/web/`，顶层 `subagent` 的 command/manager 位于 `pi/subagent/`。

> **`web` 已升为顶层一等工具**(与 `app` 同级,**不是** `app` 的子域):`pi/tools/web.ts` 路由独立的 `webCommands`,`app web ...` 已废弃。上表只列 `app` 的子命令;`web` 的子命令(`search` / `image` / `fetch` 只读 + `download` 产出型)结构同形,见 `appcmd/builtins/web/`。**`read-html` 已剥离**:本地 HTML 读取并入 `read` 工具的 HTML backend(`tools/read/backends/html.ts`,query 轴 `?mode=outline|section|selector`),online HTML 用 `web fetch`。

> **`web fetch` 定位边界（headless 渲染抓取）**：HTML 默认走**隐藏 `BrowserWindow({show:false})` headless 渲染 + 注入提取**（Readability + turndown → Markdown，JS 渲染后的真实正文），共享 `lib/research/extract/` 提取链；非 HTML（JSON/YAML/MD/XML/plain）走 raw fetch 透传，`--raw` 是 HTML 的旧纯文本逃生口。它是 **LLM 自动**的渲染抓取，比 `web research`（人审授权）更激进，仅用于公开页面正文提取：内存 partition（不落盘）、不搬用户 cookie、权限全 deny、拦下载、不绕 CAPTCHA/风控/付费墙。详见 [`lib/research/extract/ai.prompt.md`](../src/main/lib/research/extract/ai.prompt.md)。
...
| 9d | URI 三层统一 message schema 阶段(`unifed-uri.md` Phase C):`FileContentPart` / `OfficeContentPart` / `OthersContentPart` 的 `filePath` 字段物理改名 `fileUri` + `FileUri` newtype brand;`write` 工具 args/result `filePath → fileUri`(JSON schema 同步);`download` 工具 result 同改;委派运行的 deliverable collector 读新字段。**零兼容**:旧 jsonl 里的 `filePath` 字段在新代码下不被消费 |
| 10 | `present_deliverables` 工具下线 + `download_file` LLM-visible 名重命名为 `download`:UI 端拆掉 `presentedFiles` 字段、`attachPresentedFilesToFollowingAssistant` 跨消息搬运逻辑,统一走 assistant 文本里的 URI/abs path 抽取;system prompt 加 `FINAL DELIVERABLES — MENTION CREATED FILES IN YOUR FINAL REPLY` 段强约束。委派运行只保留 `write` / `download` 自动跟踪兜底审计,父 agent 收到的 `Deliverables` 段不变。 |
| 11 | `web` 域重构:① `download` 顶层工具降为 `web download` 子命令(kernel 移到 `appcmd/builtins/web/kernel/download.ts`),deliverable 审计改走**结构化回流** —— `AppCmdContext.addDeliverable` → `AppCmdInternalResult.deliverables` → facade `ToolResult.deliverables` → `executeToolCall` → delegated run path，不再 hardcode `download` toolName(`write` 仍走 args.fileUri)。② `web read-html` 子命令删除,本地 HTML 读取并入 `read` 工具新增 HTML backend(`read/backends/html.ts` + `htmlReader.ts`),走 **URI query 轴**(`?mode=...`,gate 在 `.html`/`.htm` 扩展名,`URLSearchParams` 解析保 CSS `#` 字面);`:sel`(行/页/raw)= 当纯文本读;online HTML → `web fetch`。 |

### 10.3 新增域时的纪律(若将来再发生)

- 业务内核必须**与 CLI 同住** `appcmd/builtins/app/<domain>/kernel/`(`web` 域为 `appcmd/builtins/web/kernel/`),**禁止**新建 `pi/tools/<domain>/`。
- 同批次完成:内核搬迁 / 旧 LocalTool 物理删 / renderer + IPC 消费者切换 / 测试 / 文档同步;**禁止** `@deprecated` 兼容壳(项目规则 `ts-no-deprecated-leftovers`)。
- 文件布局照 §6 范式(`builtins/app/hello/` / `builtins/app/mcp/` 是活模板);共享 flag 照 `appcmd/_commonFlags.ts`;fixture 范式见 `__tests__/app/<域>/_fixture.ts`(`vi.hoisted` + `export { mocks }`)。
...
- 上游:[Agent Loop(pi)](agent-loop.md) —— `pi/tool.ts` / `pi/session/` per-turn 构建 catalog 后透传 ctx 到 `executeToolCall`,`app` 与其它 LocalTool 走同一路径。
