...
<!-- Last verified: 2026-07-16 (Step 9：顶层 subagent 已替代旧 app subagent backend) -->
## 1. 范围
...
| 11 | `web` 域重构:① `download` 顶层工具降为 `web download` 子命令(kernel 移到 `appcmd/builtins/web/kernel/download.ts`),deliverable 审计改走**结构化回流** —— `AppCmdContext.addDeliverable` → `AppCmdInternalResult.deliverables` → facade `ToolResult.deliverables` → `executeToolCall` → sub-agent hook,`subAgentChat.trackDeliverables` 不再 hardcode `download` toolName(`write` 仍走 args.fileUri)。② `web read-html` 子命令删除,本地 HTML 读取并入 `read` 工具新增 HTML backend(`read/backends/html.ts` + `htmlReader.ts`),走 **URI query 轴**(`?mode=...`,gate 在 `.html`/`.htm` 扩展名,`URLSearchParams` 解析保 CSS `#` 字面);`:sel`(行/页/raw)= 当纯文本读;online HTML → `web fetch`。 |
