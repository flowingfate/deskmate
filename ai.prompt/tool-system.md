...
所有 LocalTool → AppCommand 迁移已完成,顶层 LLM-visible 工具为 **8**
(`read` / `write` / `find` / `search` / `shell` / `ask` / `app` / `web`)。
其中 `app` 与 `web` 是两个**完全对等**的 router facade —— 都是
`makeCommandFacade(makeRouterCommand({ name, synopsis, registry }))`,各自路由自己的注册表
(`appcmd/builtins/app/index.ts` 的 `appCommands` / `appcmd/builtins/web/index.ts` 的 `webCommands`),
路由 / help / 描述索引逻辑同一份,差异仅注册表装了谁。
...
实现仓:`app` 域成员在 `appcmd/builtins/app/<domain>/`,`web` 域成员在 `appcmd/builtins/web/`,各域业务内核 `kernel/*.ts` + CLI 同住。
