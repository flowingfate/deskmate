...
> **现状**:Phase 8a 完成后顶层 LLM-visible 工具数从 14 收敛到 **10**(`read_file` / `read_office_file` / `write` / `find` / `search` / `shell` / `ask` / `download_file` / `present_deliverables` / `app`,加 lazy-only 暴露的 `read_office_file`)。`edit` 尚未独立;`read_file` + `read_office_file` 的统一化、`write` 与 `download_file` 合并是后续 phase 目标(详见 §9)。
...
所有 LocalTool → AppCommand 迁移已完成,顶层 LLM-visible 工具从 38 降到 **9**
(`read` / `write` / `find` / `search` / `shell` / `ask` / `download_file` /
`present_deliverables` / `app`)。
...
| 9d | URI 三层统一 message schema 阶段(`unifed-uri.md` Phase C):`FileContentPart` / `OfficeContentPart` / `OthersContentPart` 的 `filePath` 字段物理改名 `fileUri` + `FileUri` newtype brand;`write` 工具 args/result `filePath → fileUri`(JSON schema 同步);`present_deliverables` args `filePaths → fileUris`;`download_file` result 同改;`subAgentChat.trackDeliverables` 读新字段。**零兼容**:旧 jsonl 里的 `filePath` 字段在新代码下不被消费 |
