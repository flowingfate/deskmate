...
BaseSession (abstract)                  src/main/pi/session.ts:78
...
6. **iter cap = 30**:`MAX_TURN_ITERATIONS`(session.ts),模型刷工具死循环 / 模型生成空 toolCall 时兜底。
...
if persistSession.config.turn?.status === 'running':
    pendingResume = planResume(messages)        // pi/resume.ts 纯函数
else:
...
**overflow 兜底**(session.ts:252):pi.stream 抛错 → `classifyError === 'overflow'`(模式见 `errors.ts:OVERFLOW_PATTERNS`,跨 anthropic / google / openai-compat)→ `doCompress(true)` 强制再压一次 → forced.applied 为 false 时(已经压无可压)抛原始错误 → 否则用 forced.llmContext 重新 streamOneRound 一次。
...
**推流契约**(仅 RegularSession,详见 `session.ts:606`):`toolcall_delta` → `tool_call` chunk(argument 增量);`toolcall_end` → 再发一次完整 JSON(应对 unicode 切断);`handleToolCalls` 完成 → `tool_result` chunk;turn 末尾 → `complete` chunk;`setStatus` → `status_changed` chunk。
...
- 历史路径(`ghcModelsManager.getModelById` + `ghcToPiModel`)已经下线,改由 pi-ai 内置 catalog + `modifyModels` hook 承担;`providers/ghc/` 还在(`GHC_CONFIG` 给 OAuth 流程用、`ghcModelsManager` 还在启动时拉 `/models` 维护本地缓存),但 model registry **不再消费** 它的数据。
...
chat.turn                                                       session.ts:179
   ├─ derive(mod: 'chat.compress')
   │   └─ FullModeCompressor → contextCompressionLlmSummarizer  → chat.compress.summary
   ├─ derive(mod: 'chat.llm')                                    session.ts:561
   │   └─ pi.stream(...)
   └─ derive(mod: 'chat.tool', toolName, callId)                tool.ts:127
...
## 12. 错误分类(`pi/errors.ts`)

`classifyError(err) → 'overflow' | 'auth' | 'rateLimit' | 'network' | 'other'`,基于错误文本的模糊匹配(pi 没有结构化错误码字段)。每个 kind 的 pattern 列表见 `errors.ts:16-51`,覆盖 anthropic / openai / google 实际错误文本。
...
| 想干啥 | 看哪 |
|---|---|
| 改 turn loop | `session.ts` 是单一权威;新形态在 `BaseSession` 上扩子类,不要回到两条平行 loop |
| 改压缩阈值 | `compression.ts:DEFAULT_COMPRESSION_THRESHOLD`(0.85);sub-agent 等可注入更激进值 |
| 改 iter 上限 | `session.ts:MAX_TURN_ITERATIONS`(30) |
...
| 加 chunk 类型 | `shared/types/streamingTypes.ts` 加 type → `session.ts:streamOneRound` / `handleToolCalls` 发 → renderer chunk 处理 |
| 加错误模式 | `errors.ts:<KIND>_PATTERNS` 数组追加 regex,注释里写哪个 provider |
