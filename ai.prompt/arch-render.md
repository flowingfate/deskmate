...
渲染进程支撑**三个独立的 Electron `BrowserWindow`**，每个都有自己的 Vite 入口、HTML 文件和 React 树。它们共享 `src/renderer/` 作为代码池，但挂载完全独立的组件树。
...
| 3 | **工具栏** | `toolbar.tsx` / `toolbar.html` | 无 — 渲染 `components/pages/ToolBarPage.tsx` 并复用主窗口组件 | 可在屏幕任意位置弹出的浮动上下文菜单工具栏。低频功能，与主窗口代码放在一起而非单独拆出。 |

截图和工具栏入口各自通过 `createRoot` 渲染单个根节点，完全绕过 `<App />` 和路由。
...
├── toolbar.tsx, toolbar.html          # 工具栏窗口
...
1. **工具栏回退** — 如果 `window.location.hash === '#/toolbar'`，渲染 `<ToolBarPage />` 并跳过其他所有内容。（仅用于在主窗口 BrowserWindow 内打开工具栏 UI 的情况；独立工具栏窗口根本不加载 `App.tsx`。）
2. **就绪门控** — 在 `electronAPI.isReady()` / `onAppReady` 之前阻塞 provider 栈。ProfileDataProvider 在主进程服务（profile 缓存、MCP 运行时）完成启动前不能触发 IPC，否则首次调用会与 bootstrap 竞争并产生虚假错误。
3. **Provider 栈**（外 → 内）：
...
4. **AppContent** 挂载 `HashRouter`、`WindowsTitleBar`、`WindowZoomHotkeys`、`McpAuthConsentDialog`、`RequestOAuthClientIdDialog`、`<AppRoutes />`。
...
| `/settings/*` | `SettingsPage` | mcp / runtime / skills / sub-agents (FF) / memory / sync / about / archived / remote-channel (FF) |
...
| 聊天 UI | `src/renderer/components/chat/` | ChatView/ChatViewContent/ChatContainer/ChatRenderItem + message 子目录（MarkdownView/AssistantMessage/UserMessage/GreetingMessage/AttachmentList/CopyButton 等）+ ComposeInput/EditInlineInput + agent-editor（8 个 tab）+ toolCallViews + chat-input 子组件 + workspace | [ai.prompt.md](../src/renderer/components/chat/ai.prompt.md) |
...
| `components/msalAuth/` | 基于 MSAL 浏览器的 Microsoft Graph 认证流程 |
| `components/pages/` | 顶层路由页面：Startup / SignIn（占位）/ DataLoading / Agent / Settings / **ToolBar** |
| `components/settings/` | 所有 `/settings/*` 面板（每个类别一个） |
| `components/skills/` | Skills 列表 + 库安装 |
| `components/subAgents/` | 子 Agent CRUD + 库 |
...
| `ipc/` | 每个子系统一个文件：`remoteChannel`、`scheduler`、`screenshot-main`、`screenshot-overlay`、`teams` |
| `states/` | 应用级 atom（如 `left-nav.atom.ts`）。其他状态应放在消费组件旁边，命名为 `*.atom.ts`。见 §8。 |
| `types/` | `mcpTypes`、`profileTypes`、`startupValidationTypes`、`agentContextTypes`、`global.d.ts`（声明 `window.electronAPI`） |
...
- **`agents.atom`** —— hydrate 时一次性拉满（`getSnapshot` 返回 `AgentRecord[]`，main 端**不**读 AGENT.md）。包含 `id / name / version / model / emoji / avatar / createdAt / updatedAt`。sidebar / chat header / model selector / 列表渲染只需这层。
- **`agentDetail.atom`** —— 按 agentId 懒读（`persistApi.getAgentDetail(id)` 触发 main 端单文件 read AGENT.md）。包含 `systemPrompt / thinkingLevel / knowledge / mcpServers / skills / subAgents / zeroStates`。agent editor / apply-to-dialog / context-menu skill list 等才需要。
...
1. 只渲染 name / emoji / model 等 hot 字段 → `useAgentById(id)` / `useAgents()`（拿 record）。
2. 渲染 systemPrompt / mcp / skills / zeroStates / 等 cold 字段 → `useAgentDetail(id)`；detail 还没到位时 hook 返 `null`，组件用骨架/默认值兜底。
...
| agent 编辑器（系统提示、MCP、skills） | 聊天 UI → agent-editor | `src/renderer/components/chat/agent-editor/` |
...
| 子 Agent UI | 子 Agent UI | `src/renderer/components/subAgents/` |
...
| 工具栏窗口 | ToolBar 入口 | `src/renderer/toolbar.tsx`，`components/pages/ToolBarPage.tsx` |
| 截图窗口（裁剪、标注 UI） | 截图入口 + 文件夹 | `src/renderer/screenshot.tsx`，`src/renderer/screenshot/` |
| Microsoft（MSAL）登录流程 | msalAuth | `src/renderer/components/msalAuth/` |
...
- 3 个 Vite 入口 → 3 个 HTML 页面 → 3 个 `BrowserWindow`（见 §1）。
