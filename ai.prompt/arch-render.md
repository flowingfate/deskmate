<!-- Last verified: 2026-07-20 (renderer error logging boundary) -->
...
`HashRouter`（Electron 通过 `file://` 加载，必须使用此路由器）。启动期无强制登录漏斗：根路径直接进入 `/agent`（基于当前 active profile，guest 也能正常工作）。运行时若 chat 调用所需 provider 凭证缺失，由 `pi/model.ts` 抛运行时错误并由 UI 引导至 `Settings → Provider`。`/login` 暂为占位 UI（登录子系统待重做）。

| 路径 | 页面 | 说明 |
|------|------|------|
| `/` | — | `Navigate to="/agent"`（不再跳 `/login`） |
| `/login` | `SignInPage` | **占位 UI**（renderer 侧老 GHC 登录已下架，仅引导用户回到 `/agent` 或去 `Settings → Provider`）|
| `/agent[/:agentId/:sessionId]` | `ChatView` | 主聊天界面；访问 `/agent/:agentId` 时 loader 分配新 session ID 并重定向到完整 URL |
| `/agent/:agentId/settings/*` | `AgentEditingView` | Agent 编辑器（基础 / 系统提示 / mcp / skills / …）；loader 在首次进入时记录来源，顶部返回按钮跨 tab 回到该来源 |
| `/agent/creation/*` | agent-area | 自定义 agent 创建 + 库 |
| `/settings/*` | `SettingsPage` | mcp / runtime / skills / memory / sync / about / archived / remote-channel |

`RootLayout` 将主进程的 `navigate:to` 事件桥接到 React Router，并把每次 route change 写成结构化日志；renderer 不再拥有 crash breadcrumb/status IPC。
...
| `log/` | Renderer logger 入口（`log.info(...)`），按 level 早过滤后通过 `log:write` IPC 单向 send 到主进程；`installGlobalErrorHandlers` 将 `window.error` / `unhandledrejection` 记为 `error`，根 React ErrorBoundary 记为 `fatal`，均不生成 Crash Recorder Incident。About 页只通过受控 IPC 列出可导出的 Incident 摘要并执行显式导出。详见 [log-analysis.md](../ai.prompt/log-analysis.md) |
