...
- **`agentDetail.atom`** —— 按 agentId 懒读（`persistApi.getAgentDetail(id)` 触发 main 端单文件 read AGENT.md）。包含 `systemPrompt / thinkingLevel / tools / mcpServers / skills / subAgents / delegates / zero`。agent editor 与单 Agent 配置读取使用。
