<!-- Last verified: 2026-07-17 (Step 12：subagentRun lazy messages query 已接入) -->
...
`subagentRun` 是双向范例：`getRunData(parent)` 返回 parent-owned `SubrunDataFile`，`getRunMessages(parent)` 懒返回同一 owner 的 Domain `Message[]`，`cancelRun(parent)` 返回 explicit lifecycle/lookup outcome，`stateUpdate` 推送 `SubAgentRuntimeState`。所有调用都携带 parent Agent/session + Subrun ID；renderer 绝不传绝对路径或自行拼 persist 路径。
