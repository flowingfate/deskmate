...
import { log } from '@/log';
const logger = log.child({ mod: 'Context' });
...
class Model {
  ...
  public log() {
    logger.debug({ msg: "undo stack:", data: this.ustack });
    logger.debug({ msg: "redo stack:", data: this.rstack });
    const snapshots: Record<string, any> = {};
    this.store.forEach((state, name) => {
      snapshots[name] = state.get();
    });
    logger.debug({ msg: "current state:", data: snapshots });
  }
