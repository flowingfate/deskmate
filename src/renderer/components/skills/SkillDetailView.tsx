...
import { log } from '@/log';
const logger = log.child({ mod: 'SkillDetailView' });
  ...
  useEffect(() => {
    ...
    const loadSkillMarkdown = async () => {
      ...
      try {
        ...
        const result = await skillsApi.getSkillMarkdown(skill.name)
        ...
        logger.error({ msg: "Error loading skill markdown:", err: err })
