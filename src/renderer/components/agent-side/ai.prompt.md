...
| `jobs/RunRow.tsx` | 单 run 行：状态 icon + 标题 + 完成时间/错误；点击 navigate 到 `/job/:jobId/:sessionId`；More 按钮以 `schedule` source 触发 `ChatSessionMenuAtom`（即 Download / Delete-only 菜单） | ~95 LOC |
| `jobs/runStatusIcons.tsx` | 4 种运行状态的 SVG icon（Executing / Completed / Interrupted / Failed） | ~70 LOC |
| `jobs/utils.ts` | `getScheduledSessionDisplayState` / `describeSchedule` / `formatDateTime` / `formatRunTime` + `JobRowStatus` 类型 + `deriveJobRowStatus` (renderer 自算 expired,以 `runAt` 而非 adapter `status` 为准) | ~85 LOC |
...
### Alarm 切换契约
`AlarmToggleButton` 在 jobs ↔ sessions 间切。jobs → sessions 时若 URL 携带 `:sessionId`（无论它是 regular session 还是 job-run），就跳到 `/agent/:agentId/:sessionId`，让 ChatView 视觉连续。该 sessionId 在 `/agent/:agentId/:sessionId` 路由下被 ChatView 当作普通 session 渲染，而 ChatView 只读 `:agentId + :sessionId`，不关心父路径。
...
`kind` 决定 ChatView 的快照拉取走哪条 IPC：
- `'regular'` → `loadChatSessionSnapshot` + `markSessionRead` → 主进程 `Agent.getSession()` → `regular_sessions` 表。
- `'job-run'` → `loadJobRunSnapshot` + `markJobRunRead` → 主进程 `Agent.getJob().getRun()` → `job_runs` 表。

**两条路径在主进程完全物理隔离 —— 不存在"按 sessionId 万能取" 的混查 helper**。Cache 仍按 sessionId 索引（同一份 in-memory 容器），区别只在 snapshot 来源。
...
4. 在 runs 子屏点 RunsHeader 的 More → Delete，确认回到 `/agent/:agentId/job`。
5. 在 sessions 模式下浏览 → 选中某 session（URL `:sessionId` 段）→ 点 alarm → 进 jobs；再点 alarm → URL 跳回 `/agent/:agentId/:sessionId`，右侧 ChatView 视觉连续。
