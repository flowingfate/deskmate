 ...
 */
const buildRunAtFromRelative = (rel: { daysFromNow: number; time: string }): string => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', marginTop: '16px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <Button variant="ghost" type="button" style={radioCardStyle(mode === 'once')} onClick={() => setMode('once')}>
              ...
              <Button variant="ghost" type="button" style={radioCardStyle(mode === 'recurring')} onClick={() => setMode('recurring')}>
          ...
          <div>
            ...
            <Textarea className="min-h-[96px] resize-y" value={message} onChange={handleMessageChange} placeholder="Write the exact prompt that the agent should receive when this schedule runs." />
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                {(recurringPreset === 'monthly' || recurringPreset === 'every_n_months') && (
                  <div>
                    ...
                    <Input type="number" min={1} max={28} value={monthlyDay} onChange={(e) => setMonthlyDay(Number(e.target.value) || 1)} />
                  </div>
                )}
                ...
                {(recurringPreset === 'every_n_days' || recurringPreset === 'every_n_weeks' || recurringPreset === 'every_n_months') && (
                  <div>
                    ...
                    <Input type="number" min={1} value={everyNValue} onChange={(e) => setEveryNValue(Number(e.target.value) || 1)} />
                  </div>
                )}
              </div>
