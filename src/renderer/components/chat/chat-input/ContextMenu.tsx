...
export const ContextMenu: React.FC = () => {
  ...
  const renderOptionContent = (option: ContextOption) => {
    ...
    if (option.type === ContextMenuOptionType.File || option.type === ContextMenuOptionType.Folder
      ...
      if (hasValue) {
        ...
        // Strip `knowledge://` / `local://` URI prefixes before splitting path segments.
        const prefixes = ['knowledge://', 'local://'];
  ...
  const getIcon = (option: ContextOption): string => {
    ...
    // Knowledge / Local file type icons
    if (option.type === ContextMenuOptionType.Knowledge) {
      return '📚';
    }

    if (option.type === ContextMenuOptionType.Local) {
      return '💬';
    }
