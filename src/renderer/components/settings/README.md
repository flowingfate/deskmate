...
This document describes how to add a new Settings page in Deskmate, and the unified design and implementation conventions that all Settings pages must follow.
...
Profile-level 配置写路径走 IPC 到 main 进程的 `persist/` 模块（详见 [`src/main/persist/ai.prompt.md`](../../../main/persist/ai.prompt.md)）；renderer 侧通过 atom 订阅 `persist:*` 通道获取最新缓存，不再走 `profileDataManager`。
