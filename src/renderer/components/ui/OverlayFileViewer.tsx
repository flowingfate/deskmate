...
import './OverlayFileViewer.scss';
import { inlinePreviewCoordinator } from '../chat/ChatInlinePreviewOverlay';
import { resolveFileDescriptorUrl } from '@/lib/internalUrls';
import { log } from '@/log';
const logger = log.child({ mod: 'OverlayFileViewer' });
...
function isOverlayFileDescriptor(value: unknown): value is OverlayFileDescriptor {
...
export const OverlayFileViewer: React.FC<OverlayFileViewerProps> = ({
  ...
  useEffect(() => {
    const handleOpenFileViewer = (event: Event) => {
      // 聊天页 inline 预览挂载时让出处理权;ChatInlinePreviewOverlay 自己会处理。
      if (inlinePreviewCoordinator.mounted) return;
      const detail: unknown = (event as CustomEvent<unknown>).detail;
      const file = detail && typeof detail === 'object'
        ? (detail as { file?: unknown }).file
        : undefined;
      if (!isOverlayFileDescriptor(file)) return;
      // `file.url` 可能是 `local://` / `knowledge://` URI(由 LLM 输出
      // 或 sandbox attach 流程产生)。URI → 绝对路径再开 viewer;非 URI 透传。
      void (async () => {
        try {
          const resolved = await resolveFileDescriptorUrl(file);
          actions.open(resolved);
        } catch (err) {
          logger.warn({ msg: 'Failed to resolve file URI for viewer', err, url: file.url });
        }
      })();
    };

    window.addEventListener('fileViewer:open', handleOpenFileViewer);
    return () => {
      window.removeEventListener('fileViewer:open', handleOpenFileViewer);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (isLocalFile(file.url)) {
      ...
      fsApi.stat(localPath).then((result) => {
      ...
      if (isLocalFile(file.url)) {
        ...
        const doRead = async () => {
          try {
            ...
            const statResult = await fsApi.stat(localPath);
            ...
            if (cancelled) return;
            logger.error({ msg: "Failed to load local text:", err: err });
        ...
        fetch(file.url)
          ...
          .catch((err) => {
            if (cancelled) return;
            logger.error({ msg: "Failed to load remote text:", err: err });
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      logger.error({ msg: "Failed to save file:", err: err });
  ...
  }, [isOpen, isEditing, handleSave, toggleFullscreen]);
  ...
  const handleDownload = useCallback(() => {
    ...
    try {
      if (isLocalFile(file.url)) {
        ...
        const localPath = getLocalPath(file.url);
        workspaceApi.openPath(localPath);
      } else {
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent
        ...
        hideCloseButton
        className={cn(
          "file-viewer-content max-w-none p-0 gap-0 border-0",
          isFullscreen && "file-viewer-content-fullscreen"
        )}
        onEscapeKeyDown={(e) => {
          e.preventDefault();
          if (document.fullscreenElement === contentRef.current) return;
          if (isEditing) {
            handleCancelEdit();
          } else {
            handleClose();
        ...
        <div className="file-viewer-header">
          ...
          <div className="file-viewer-file-info">
            ...
            <div className="file-viewer-file-meta">
              ...
              <Badge className={`file-viewer-mode-badge ${isEditing ? 'file-viewer-mode-edit' : 'file-viewer-mode-preview'}`}>
