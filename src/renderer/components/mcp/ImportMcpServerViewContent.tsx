/**
 * ImportMcpServerViewContent Component
 * Main content area containing the MCP servers import functionality
 */

import React, { useState, useCallback, useEffect } from 'react'
import { Checkbox } from '@/shadcn/checkbox'
import './ImportMcpServerView.scss'
import { useMcpRuntimeServers, refreshMcpRuntime } from '@/states/mcpRuntime.atom'
import { useToast } from '../ui/ToastProvider'
import { getPlatformInfo } from '../../lib/mcp/platformDetector'
import { readFileContent } from '../../lib/utilities/fileSystemUtils'
import { detectMcpConfigs } from '../../lib/mcp/McpConfigDetector'
...
interface ImportMcpServerViewContentProps {
  onImportComplete?: (importedCount: number) => void
}

const ImportMcpServerViewContent: React.FC<ImportMcpServerViewContentProps> = ({
  onImportComplete
}) => {
  ...
  const handleAutoDetect = useCallback(async (currentExistingNames: string[]) => {
    ...
    try {
      ...
      const detectionResult = await detectMcpConfigs()
      ...
      if (!detectionResult.success) {
        setDetectedConfig({
          ...
          exists: false,
          serverCount: 0,
          error: detectionResult.error || 'Failed to scan MCP configuration files.'
        })
        return
      }
      ...
      if (!validConfigFile) {
        ...
        if (firstExistingConfig) {
          setDetectedConfig({
            ...
            exists: true,
            serverCount: 0,
            error: firstExistingConfig.error || 'Found an MCP configuration file, but it does not contain a supported MCP server configuration.'
          })
          return
        }
        ...
        setDetectedConfig({
          ...
          exists: false,
          serverCount: 0,
          error: 'No MCP configuration file found in the default user or profile locations.'
        })
        return
      }
    ...
    try {
      ...
      for (const server of serversToImport) {
        try {
          ...
          // Convert to Deskmate.app format
          const deskmateConfig: DeskmateAppMCPServerConfig = {
            name: finalName,
          ...
          if (server.hasConflict && importOptions.conflictResolution === 'overwrite') {
            ...
            result = await McpOps.update(server.name, deskmateConfig)
            ...
            result = await McpOps.add(deskmateConfig)
  ...
  const handleTestImport = useCallback(async () => {
    ...
    try {
      ...
      showSuccess('Import flow is working!')
      onImportComplete?.(1)
    } catch (error) {
  ...
  return (
    <div className="mcp-importer-content">
      {/* Auto-detection section */}
      <div className="detection-section">
        {isScanning ? (
          <div className="scanning-status">
            <span className="spinner">🔍</span>
            <span>Scanning for MCP configuration...</span>
          </div>
        ) : detectedConfig ? (
            ...
            {detectedConfig.exists && detectedConfig.serverCount > 0 ? (
              <>
                <div className="success-message">
                  ✅ Scan successful! Found {detectedConfig.serverCount} MCP servers in the configuration
                </div>
              ...
              <>
                <div className="warning-message">
                  ⚠️ Found a configuration file but no MCP servers detected
                </div>
                <div className="detection-path">
                  <strong>Configuration file path:</strong> {detectedConfig.path}
                </div>
                <div className="help-message">
                  Please ensure your MCP client has servers configured in `mcp.json` / `settings.json`.
                </div>
              </>
            ) : (
              <>
                ...
                <div className="help-message">
                  <h4>Solutions:</h4>
                  <ul>
                    <li>Ensure your MCP client (such as Claude Desktop, Cursor, or another supported app) is installed</li>
                    <li>Check that the chosen client has MCP servers configured</li>
                ...
                >
                  <label className="server-checkbox">
                    ...
                    <span className="server-info">
                      ...
                      {server.hasConflict && <Badge variant="destructive">Name conflict!</Badge>}
                    </span>
                  </label>
                  <div
                    className="server-info-icon"
                    title="View original imported configuration"
            ...
            <div className="option-group">
              <label className="checkbox-label">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setImportOptions(prev => ({ ...prev, validateBeforeImport: !!checked }))}
                />
        ...
        <div className="status-section">
          {!detectedConfig ? (
            <>
              ...
              <p>Opening this dialog will automatically scan for MCP configuration files.</p>
            </>
          ) : detectedConfig.exists && detectedConfig.serverCount === 0 ? (
            <>
              <h3>No MCP servers found</h3>
              <p>Found a configuration file, but it contains no MCP server configurations.</p>
            </>
          ) : (
            <>
              <h3>Ready to import</h3>
              <p>MCP import feature is ready, waiting for configuration file detection.</p>
            </>
          )}
        </div>
      )}
      ...
      {tooltipServer && tooltipPosition && (
        ...
        >
          <div className="tooltip-header">Original imported configuration:</div>
...
export default ImportMcpServerViewContent