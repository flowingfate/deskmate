...
export class FileAttachmentProcessor {
  ...
  static async processFileFromPath(filePath: string): Promise<ChatReferenceFileData> {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        const result = await fsApi.getFileMetadata(filePath);
        ...
        if (result.success) {
          ...
          throw new Error(!result.success ? result.error : 'Failed to get file metadata');
        }
      } catch (error) {
  ...
  /**
   * Estimate token count for text using the rough character-density heuristic length * 3 / 4.
   */
