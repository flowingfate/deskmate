import { log } from '@/log';
const logger = log.child({ mod: 'UserInputParser' });
...
export const parseUserInputVariable = (key: string, value: string): UserInputVariable | null => {
  ...
  if (!match) {
    logger.warn({ msg: `Invalid user input variable format: ${value}. Expected format: @USER_INPUT_[TYPE]_[CONTROL]_[REQUIRED|OPTIONAL]_{KEYNAME}` });
    return null;
  }
...
const validateAndCreateVariable = (
  ...
  if (!validTypes.includes(typeStr)) {
    logger.warn({ msg: `Invalid type in user input variable: ${typeStr}` });
    return null;
  }

  const validControls = ['FOLDER', 'FILE', 'TEXT'];
  if (!validControls.includes(controlStr)) {
    logger.warn({ msg: `Invalid control in user input variable: ${controlStr}` });
    return null;
  }
  ...
  if ((typeStr === 'INT' || typeStr === 'DOUBLE' || typeStr === 'BOOLEAN') && controlStr !== 'TEXT') {
    logger.warn({ msg: `Type ${typeStr} can only be used with TEXT control, got ${controlStr}` });
    return null;
  }
