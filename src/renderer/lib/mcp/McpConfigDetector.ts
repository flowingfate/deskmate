 ...
 * Detects and validates MCP configuration files across platforms
 */

import { getPlatformInfo, getCurrentPlatform, getMcpConfigPaths } from './platformDetector'
...
export interface McpConfigDetectionResult {
  success: boolean
  platform: string
  isSupported: boolean
...
/**
 * Detect MCP configuration files for the current platform
 */
export async function detectMcpConfigs(): Promise<McpConfigDetectionResult> {
  try {
    ...
    if (!platformInfo.isSupported) {
      return {
        ...
        configFiles: [],
        totalServersFound: 0,
        error: `Platform ${platformInfo.platform} is not currently supported for MCP config import`
      }
    }

    // Get all candidate MCP config paths for current platform (prioritized order)
 ...
 */
export async function detectMcpConfigFile(): Promise<string | null> {
  try {
    const platformInfo = getPlatformInfo()
...
/**
 * Detect an MCP config file from a custom path
 */
export async function detectCustomConfigFile(filePath: string): Promise<McpConfigFile> {
...
export function getPlatformDetectionInfo() {
  ...
  return {
    ...
    standardConfigPath: platformInfo.mcpConfigPath,
...
function getDetectionStrategy(platform: string): string {
  switch (platform) {
    case 'macOS':
      return 'Scan multiple MCP client config paths (standard, Insiders, OSS, Homebrew) for mcp.json and settings.json files'
    case 'Windows':
      return 'Scan multiple MCP client config paths (standard, Insiders, OSS, portable, system-level) for mcp.json files'
    case 'Linux':
      return 'Scan multiple MCP client config paths for settings.json files (future support)'
    default:
