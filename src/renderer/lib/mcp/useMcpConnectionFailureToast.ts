...
import { log } from '@/log';
const logger = log.child({ mod: 'UseMcpConnectionFailureToast' });
...
export const useMcpConnectionFailureToast = () => {
  ...
  const clearFailedConnection = useCallback((serverName: string) => {
    ...
    if (failed) {
      logger.debug({ msg: `Clearing toast for "${serverName}", toastId: ${failed.toastId}` })
  ...
  const handleReconnect = useCallback(async (serverName: string) => {
    try {
      ...
      if (!result.success) {
        logger.error({ msg: "Reconnect failed:", err: result.error })
      }
    } catch (error) {
      logger.error({ msg: "Reconnect error:", err: error })
    }
  ...
  const handleConnectionFailure = useCallback((serverName: string, error: string) => {
    logger.debug({ msg: "Connection failure:", err: error, data: serverName })

    // Avoid duplicate notifications for the same server
    if (failedConnectionsRef.current.has(serverName)) {
      logger.debug({ msg: "Already notified for:", data: serverName })
      return
    }
    ...
    logger.debug({ msg: "Created toast with id:", data: actualToastId })
  ...
  useEffect(() => {
    logger.debug({ msg: "Setting up connection failure listener" })
    ...
    const unsubscribeData = mcpClientCacheManager.subscribe((data) => {
      ...
      failedConnectionsRef.current.forEach((failed, serverName) => {
        ...
        if (server && server.status !== 'error') {
          logger.debug({ msg: `Server "${serverName}" status changed to "${server.status}", auto-dismissing toast` })
          clearFailedConnection(serverName)
        }
      })
    })
    ...
    return () => {
      logger.debug({ msg: "Cleaning up connection failure listener" })
      unsubscribeFailure()
      unsubscribeData()
