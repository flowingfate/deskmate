...
// MCP config export helpers (settings.json-style and mcp.json-style)

import { DeskmateAppMCPServerConfig } from '../../types/mcpTypes'

/**
 * Convert Deskmate.app server config to settings.json-style MCP format.
 */
export function formatToMcpSettings(serverConfigs: DeskmateAppMCPServerConfig[]): string {
...
/**
 * Convert Deskmate.app server config to mcp.json-style MCP format.
 */
export function formatToMcpJson(serverConfigs: DeskmateAppMCPServerConfig[]): string {
  const servers: Record<string, any> = {}
...
/**
 * Convert a single Deskmate.app config to a server-entry object suitable for export.
 */
export function convertDeskmateToMcpServerEntry(config: DeskmateAppMCPServerConfig): any {
  const entry: any = {}
...
/**
 * Parse an imported MCP configuration string into the internal config format.
 */
export function parseMcpImportInput(
  input: string,
  ...
  try {
    ...
    const transportType = convertMcpImportTransportType(firstServerConfig)
    ...
    return {
      ...
      data: {
        ...
        detectedFormat: `MCP ${format} format`
      }
    }
...
/**
 * Convert an imported transport-type field to the internal transport type.
 */
function convertMcpImportTransportType(entry: any): 'stdio' | 'sse' | 'StreamableHttp' {
  if (entry.type) {
    switch (entry.type.toLowerCase()) {
      ...
      case 'streamablehttp':
        ...
        if (entry.url && entry.url.endsWith('/sse')) {
          return 'sse'
        }
        return 'StreamableHttp'
      default:
  ...
  if (entry.url) {
    ...
    return entry.url.endsWith('/sse') ? 'sse' : 'StreamableHttp'
  }
...
/**
 * Validate an imported MCP configuration string for the given format.
 */
export function validateMcpImportFormat(input: string, format: 'settings.json' | 'mcp.json'): {
  isValid: boolean
  errors: string[]
  serverCount: number
} {
