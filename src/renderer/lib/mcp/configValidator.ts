...
import {
  DeskmateAppMCPServerConfig,
  ImportValidationResult,
  ImportedMcpServerConfig
} from '../../types/mcpTypes'

export interface ValidationRule {
  name: string
  validate: (config: DeskmateAppMCPServerConfig) => ValidationRuleResult
 ...
 */
export function validateMcpServerConfig(config: DeskmateAppMCPServerConfig): ConfigValidationReport {
  const errors: string[] = []
 ...
 */
export function validateBatchImport(configs: DeskmateAppMCPServerConfig[]): ImportValidationResult {
...
function calculateQualityScore(
  config: DeskmateAppMCPServerConfig,
  errors: string[],
  warnings: string[]
): number {
  let score = 100
 ...
 */
function checkForDuplicateNames(configs: DeskmateAppMCPServerConfig[]): string[] {
  const errors: string[] = []
 ...
 */
function checkForConflicts(configs: DeskmateAppMCPServerConfig[]): string[] {
  const warnings: string[] = []
...
/**
 * Validate imported MCP server entry before conversion
 */
export function validateMcpConfigBeforeImport(
  serverName: string,
  importedConfig: ImportedMcpServerConfig
): ValidationRuleResult {
  // Check if configuration is disabled
  if (importedConfig.disabled === true) {
    return {
      passed: false,
      message: 'Server is disabled in the source configuration'
    }
  }

  // Check if required fields are present
  const hasStdioConfig = !!(importedConfig.command || importedConfig.args)
  const hasHttpConfig = !!importedConfig.url
  ...
  if (importedConfig.type === 'stdio' || hasStdioConfig) {
    if (!importedConfig.command) {
      return {
        passed: false,
  ...
  if (importedConfig.url) {
    try {
      new URL(importedConfig.url)
    } catch {
      return {
        passed: false,
        message: 'Invalid URL format'
