...
export interface MCPServerExtended {
  ...
  url: string
  in_use: boolean
  /** MCP server version (user-defined) */
  version?: string
...
export class MCPClientCacheManager {
  ...
  private handleServerStatesUpdate(serverStates: any[]): void {
    if (!serverStates || !Array.isArray(serverStates)) {
      logger.warn({ msg: "Invalid server states received" })
      return
    }
    ...
    this.cache.servers.forEach(server => {
      ...
      if (runtimeState) {
        ...
        // Server has no runtime state — mark as disconnected
    ...
    if (connectionFailures.length > 0) {
      connectionFailures.forEach(failure => {
        logger.debug({ msg: "Connection failure detected", serverName: failure.serverName, err: failure.error })
  ...
  private notifyConnectionFailure(serverName: string, error: string): void {
    this.connectionFailureListeners.forEach(listener => {
      try {
        ...
        logger.error({ msg: "Connection failure listener error:", err: err })
      }
    })
  }
  ...
  private performNotification(): void {
    ...
    this.listeners.forEach(listener => {
      try {
        ...
        logger.error({ msg: "Listener error:", err: error })
      }
    })
   ...
   * Get all MCP servers
   *
   * 返回稳定引用：仅当 cache.servers 被整体替换（handleServerStatesUpdate / updateServerConfigs）时
   * 引用才变化。用于 useSyncExternalStore 的 getSnapshot —— 若每次返回新数组会触发 React 无限重渲染
  ...
  cleanup(): void {
    ...
    logger.debug({ msg: "🔔 Notifying listeners of cleared cache" })
    this.notifyListeners()
