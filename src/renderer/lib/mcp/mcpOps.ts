...
export class McpOps {
   ...
   * Now handled through ProfileCacheManager instead of direct IPC calls
   */
  static async add(serverConfig: DeskmateAppMCPServerConfig): Promise<McpOpsResult<string>> {
    try {
      ...
      const result = await mcpApi.addServer(serverConfig.name, serverConfig);
      return result;
    } catch (error) {
      return {
        success: false,
   ...
   * Now handled through ProfileCacheManager instead of direct IPC calls
   */
  static async update(serverName: string, serverConfig: DeskmateAppMCPServerConfig): Promise<McpOpsResult<string>> {
    try {
      if (!serverName || !serverName.trim()) {
        return {
          success: false,
      ...
      const result = await mcpApi.updateServer(serverName, serverConfig);
      return result;
    } catch (error) {
      return {
        success: false,
   ...
   */
  static validate(serverName: string, serverConfig: DeskmateAppMCPServerConfig): McpServerValidationResult {
    try {
      ...
      const configToValidate: DeskmateAppMCPServerConfig = {
        ...serverConfig,
        name: serverName
      };
   ...
   */
  static validateTransportConfig(transport: 'stdio' | 'sse' | 'StreamableHttp', config: Partial<DeskmateAppMCPServerConfig>): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
