...
export class UserInputPlaceholderParser {
  ...
  parseSinglePlaceholder(key: string, value: string): UserInputField | null {
    ...
    if (!match) {
      logger.warn({ msg: `[UserInputPlaceholderParser] Invalid placeholder format: ${value}. Expected: @USER_INPUT_[TYPE]_[CONTROL]_[REQUIRED|OPTIONAL]_{KEYNAME}` });
      return null;
    }
    ...
    if (!VALID_TYPES.includes(typeStr as any)) {
      logger.warn({ msg: `[UserInputPlaceholderParser] Invalid type: ${typeStr}. Valid types: ${VALID_TYPES.join(', ')}` });
      return null;
    }

    // Validate control
    if (!VALID_CONTROLS.includes(controlStr as any)) {
      logger.warn({ msg: `[UserInputPlaceholderParser] Invalid control: ${controlStr}. Valid controls: ${VALID_CONTROLS.join(', ')}` });
      return null;
    }
    ...
    if ((typeStr === 'INT' || typeStr === 'DOUBLE' || typeStr === 'BOOLEAN') && controlStr !== 'TEXT') {
      logger.warn({ msg: `[UserInputPlaceholderParser] Type ${typeStr} can only be used with TEXT control, got ${controlStr}` });
      return null;
    }
