...
import { getBinDir } from '@main/persist/lib/path';
...
export class PlaywrightManager {
  ...
  async checkBrowserInstalled(): Promise<BrowserCheckResult> {
    if (browserInstallChecked && browserAvailable) {
      ...
      try {
        ...
        logger.warn({ msg: `[PlaywrightManager] Cached browser available but exe no longer on disk (${exePath}), re-checking...` });
      } catch {
    ...
    try {
      logger.debug({ msg: '[PlaywrightManager] Checking if Playwright Chromium is installed...' });
      const chromium = await this._importChromium();

      const browser = await chromium.launch({ headless: true, timeout: 15_000 });
      await browser.close();

      logger.debug({ msg: '[PlaywrightManager] Playwright Chromium verified (headless launch OK)' });
      ...
      if (isNotInstalled) {
        logger.warn({ msg: `[PlaywrightManager] Chromium not installed: ${errorMsg}` });
      } else {
        logger.warn({ msg: `[PlaywrightManager] Chromium launch failed: ${errorMsg}` });
      }
  ...
  async installBrowser(): Promise<BrowserInstallResult> {
    ...
    if (installPromise) {
      logger.debug({ msg: '[PlaywrightManager] Install already in progress, waiting...' });
  ...
  private async _doInstall(): Promise<boolean> {
    logger.info({ msg: '[PlaywrightManager] Installing Playwright Chromium headless-shell...' });
    ...
    for (const strategy of strategies) {
      logger.info({ msg: `[PlaywrightManager] Attempting browser install via ${strategy.label}: ${this._formatCommand(strategy.command, strategy.args)}` });
      const result = await this._runInstallCommand(strategy.command, strategy.args, strategy.shell);

      if (result.success) {
        logger.info({ msg: `[PlaywrightManager] Chromium headless-shell installed via ${strategy.label}` });
        browserInstallChecked = false;
        browserAvailable = false;
        return true;
      }
      ...
      logger.warn({ msg: `[PlaywrightManager] Install attempt failed via ${strategy.label}: ${errorMessage}` });
      ...
      if (result.timedOut) {
        ...
        logger.warn({ msg: '[PlaywrightManager] Install timed out; cleaned lock, continuing to next strategy' });
        continue;
      }
    }

    logger.error({ msg: `[PlaywrightManager] All install strategies failed: ${errors.join(' | ')}` });
    return false;
  }
  ...
  private _getInstallStrategies(cliPath: string | null): InstallStrategy[] {
    ...
    if (cliPath) {
      ...
      logger.warn({ msg: '[PlaywrightManager] Built-in CLI not found, falling back to npx-only install flow' });
    }
  ...
  private _findInternalNodeShim(): string | null {
    try {
      ...
      logger.warn({ msg: `[PlaywrightManager] Failed to resolve internal node shim: ${error instanceof Error ? error.message : String(error)}` });
    }
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        if (child.exitCode === null) {
          ...
          logger.error({ msg: '[PlaywrightManager] Install timed out, killing process' });
          child.kill('SIGTERM');

          setTimeout(() => {
            if (child.exitCode === null) {
              logger.error({ msg: '[PlaywrightManager] Install process ignored SIGTERM, sending SIGKILL' });
              child.kill('SIGKILL');
            }
          }, 2_000);
  ...
  private _logInstallOutput(stream: 'stdout' | 'stderr', output: string): void {
    for (const line of output.split(/\r?\n/)) {
      ...
      if (
        ...
        logger.info({ msg: `[PlaywrightManager] Install ${stream}: ${trimmed}` });
      } else {
        logger.debug({ msg: `[PlaywrightManager] Install ${stream}: ${trimmed}` });
      }
    }
  }
  ...
  private _cleanupPlaywrightInstallLock(reason: string): void {
    try {
      ...
      logger.warn({ msg: `[PlaywrightManager] Removed Playwright install lock after ${reason} (${lockStats.isDirectory() ? 'directory' : 'file'}): ${lockPath}` });
    } catch (error) {
      logger.warn({ msg: `[PlaywrightManager] Failed to remove Playwright install lock after ${reason}: ${error instanceof Error ? error.message : String(error)}` });
    }
  }
  ...
  private _findPlaywrightCli(): string | null {
    ...
    const possiblePaths = [
      // dev: __dirname === <repo>/out/main → 两层 .. 回到 repo root → node_modules/...
      ...
      // playwright-core 被 asarUnpack —— 真正命中的通常是这一条
  ...
  async ensureBrowserInstalled(): Promise<BrowserCheckResult> {
    ...
    logger.info({ msg: '[PlaywrightManager] Browser not installed, auto-installing...' });
  ...
  async launchPersistentContext(options: PersistentContextOptions): Promise<BrowserContext> {
    ...
    try {
      ...
      logger.info({ msg: `[PlaywrightManager] Persistent context launched (channel=${channel}, profile=${options.profileName})` });
      return ctx;
    } catch (edgeError) {
      if (channel !== 'msedge') throw edgeError;
      logger.warn({ msg: `[PlaywrightManager] Edge not available, falling back to bundled chromium: ${edgeError instanceof Error ? edgeError.message : String(edgeError)}` });
    }
    ...
    this.activeContexts.add(ctx);
    ctx.on('close', () => this.activeContexts.delete(ctx));
    logger.info({ msg: `[PlaywrightManager] Persistent context launched (bundled chromium, profile=${options.profileName})` });
    return ctx;
  }
  ...
  private async _importChromium() {
    if (!chromium) {
      logger.error({ msg: '[PlaywrightManager] playwright-core.chromium is undefined — ' +
  ...
  async closeAll(): Promise<void> {
    ...
    for (const ctx of this.activeContexts) {
      promises.push(ctx.close().catch((e) => logger.warn({ msg: `[PlaywrightManager] Failed to close context: ${e}` })));
    }
    for (const browser of this.activeBrowsers) {
      promises.push(browser.close().catch((e) => logger.warn({ msg: `[PlaywrightManager] Failed to close browser: ${e}` })));
    }
    await Promise.allSettled(promises);
    this.activeContexts.clear();
    this.activeBrowsers.clear();
    logger.info({ msg: '[PlaywrightManager] All browsers closed' });
  }
