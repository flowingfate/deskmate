...
class FeatureFlagManager {
  ...
  public initialize(): void {
    if (this.initialized) {
      logger.debug({ msg: '[FeatureFlags] Already initialized, skipping...' });
      return;
    }

    logger.debug({ msg: '[FeatureFlags] Initializing feature flag manager...' });
    ...
    logger.debug({ msg: '[FeatureFlags] Initialization complete' });
  ...
  private parseCommandLineArgs(): void {
    ...
    logger.debug({ msg: `[FeatureFlags] Parsing command line args: ${args.join(' ')}` });
  ...
  private setFlagFromCli(name: FeatureFlagName, enabled: boolean): void {
    ...
    if (!config) {
      logger.warn({ msg: `[FeatureFlags] Unknown feature flag from CLI: ${name}` });
      return;
    }
    ...
    logger.debug({ msg: `[FeatureFlags] Set ${name}=${enabled} from CLI` });
  }
  ...
  public isEnabled(name: FeatureFlagName): boolean {
    ...
    if (!flag) {
      logger.warn({ msg: `[FeatureFlags] Unknown feature flag: ${name}` });
      return false;
    }
  ...
  private logCurrentState(): void {
    logger.debug({ msg: '[FeatureFlags] Current state:' });
    logger.debug({ msg: `  Context: isDev=${this.context.isDev}, brandName=${this.context.brandName}, platform=${this.context.platform}` });
    for (const [name, state] of Object.entries(this.flags)) {
      if (state.source !== 'default' || state.enabled) {
        logger.debug({ msg: `  - ${name}: ${state.enabled} (source: ${state.source})` });
      }
    }
  }
