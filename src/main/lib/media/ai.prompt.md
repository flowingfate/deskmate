<!-- Last verified: 2026-07-18 (ResolveContext runtime Profile injection) -->
# Media Protocol 模块
...
- `<path…>`:renderer 每段 `encodeURIComponent`;handler 每段 `decodeURIComponent`
  ...
  `<authority>://<path>`。
- `<query>`:`profile`（owner Profile ID）、`agent`(ULID)、`session`(ULID)、`mime`(URL-encoded → Content-Type)。
  protocol handler 没有 IPC sender，必须由 URL 携带 profile identity；不允许回读 `defaultProfileId`。

### per-authority 必填 query(`MEDIA_AUTHORITIES` 注册表)
| authority | 内层 scheme | 必填(除 mime 外) | 说明 |
|---|---|---|---|
| `local` | `local` | `profile` + `agent` + `session` | session 级 sandbox |
| `knowledge` | `knowledge` | `profile` + `agent` | agent 级 KB,`resolveToPath` 不消费 session |
...
3. 从 query 读取 profile identity，在 protocol 边界通过 `ProfileRegistry.require(profileId)` 解析一次 runtime Profile，并构造携带该对象的 `ResolveContext`。
...
## 注意事项
- **profileId 必须进 URL** —— `media://` 无 IPC sender，不能从 selected Profile 推断资源 owner；protocol 边界按 URI identity 精确取得 runtime Profile，InternalUrlRouter 后续只沿 context 使用它。
