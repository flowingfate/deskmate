...
export class TerminalInstance extends EventEmitter implements ITerminalInstance {
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      if (runnableShell.fallbackReason) {
        logger.warn({ msg: '[TerminalInstance] Shell fallback applied', mod: 'start', requestedShell: this.config.shell || this.platformConfig.getDefaultShell(), effectiveShell: runnableShell.shellType, reason: runnableShell.fallbackReason });
      }
   ...
   * On Windows ARM, MCP stdio transports bypass the bundled Node/Bun shims
  ...
  private async prepareEnvironment(): Promise<Record<string, string>> {
    ...
    // moves from FRE boot to here so users without MCPs pay nothing, and users
    // with MCPs see the wait under "connecting" with the right context.
    if (includeBinPath && this.type === 'mcp_transport') {
      try {
        await RuntimeManager.getInstance().ensureRuntimeForCommand(this.config.command, this.config.args);
      } catch (err) {
        logger.warn({ msg: 'Lazy runtime install failed; spawning anyway', mod: 'TerminalInstance', err });
      }
    }
  ...
  private setupMcpTransportHandlers(): void {
    ...
    stderrSplitter.on('data', (line: Buffer) => {
      ...
      if (stderrMessage) {
        logger.warn({ msg: `[${this.id} stderr] ${stderrMessage}` });
