...
The unified terminal instance manager is a cross-platform terminal process management system designed specifically for the Deskmate project to uniformly manage and coordinate all terminal operations. It supports Windows and macOS, and provides efficient process pool management, lifecycle control, and resource cleanup.
...
本地工具 [`pi/tools/executeCommand.ts`](../../pi/tools/executeCommand.ts) 通过本管理器
落地 shell 命令执行(LLM-visible name 为 `shell`;Phase 8a 之前叫 `execute_command`),
内部统一走 `getTerminalInstanceManager()` 拿单例实例并 spawn 子进程:

```typescript
// 本地工具通过 LocalTool registry 调用,内部统一走 terminal manager:
const result = await tools.execute('shell', { command, args, cwd }, ctx);
```

### StdioTransport Integration

The refactored [`StdioTransport`](src/main/lib/mcpRuntime/client/transport/StdioTransport.ts) uses the unified terminal manager:
...
### Migrating from the original `execute_command` tool (pre-Phase-8a `shell` rename)
