...
describe('CrashCaptureManager', () => {
  ...
  beforeEach(async () => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'deskmate-crash-capture-'));
    ...
    (electronMock.app.getName as Mock).mockReturnValue('deskmate-test');
    ...
    // root.ts uses `require('electron')` which the ESM `vi.mock` in tests/setup.ts
    // does not intercept; the `vi.resetModules()` call above also clears the global
    // root override. Re-anchor it on the per-test tmp userData dir so getAppRoot()
    // resolves to ${userDataDir} and getCrashesDir() == ${userDataDir}/crashes.
    const root = await import('@main/persist/lib/root');
    root.setRootForTesting(userDataDir);
  });

  afterEach(async () => {
    const root = await import('@main/persist/lib/root');
    root.setRootForTesting(null);
    fs.rmSync(tempRoot, { recursive: true, force: true });
  });

  it('captures recent logs, current run marker, and crash dumps in renderer-error bundles', async () => {
    seedLogFile(userDataDir, 'deskmate-2026-04-07.log', ['line-1', 'line-2']);
    ...
    expect(attachments.recentLogs).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ fileName: 'deskmate-2026-04-07.log', copied: true }),
    ...
    expect(fs.existsSync(path.join(bundleDir, 'recent-logs', 'deskmate-2026-04-07.tail.log'))).toBe(true);
  ...
  it('captures previous run marker and recent crash dumps for recovered unclean exits', async () => {
    ...
    seedLogFile(userDataDir, 'deskmate-2026-04-07.log', ['main-line']);
