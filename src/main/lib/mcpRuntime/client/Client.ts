...
import { TransportFactory, McpTransport } from './transport/TransportFactory';
...
/**
 * MCP client. Owns initialize handshake, tool / resource enumeration,
 * JSON-RPC plumbing, and shutdown.
 */
export class McpClientCore extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    try {
      // Create transport
      this.transport = TransportFactory.createFromConfig(this.config.name, this.config);
  ...
  private async initializeMcp(): Promise<void> {
    // No initialization timeout or retry: an MCP server may need to download
    // dependencies before it speaks protocol; timing is unpredictable.
   ...
   * An MCP server may need to download dependencies on first start, so timing is unpredictable.
   */
  ...
  private log(level: 'trace' | 'debug' | 'info' | 'warning' | 'error', message: string): void {
    ...
    if (this.logger) {
      const newLevel: 'trace' | 'debug' | 'info' | 'warn' | 'error' =
        level === 'warning' ? 'warn' : level;
      this.logger[newLevel]({ msg: message, component: 'McpClient', serverName: this.config.name });
    }
  }
}