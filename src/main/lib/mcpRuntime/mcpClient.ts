...
/** Map raw MCP tool to internal Tool shape */
...
/**
 * Adapter providing the same interface as the legacy SDK MCP client, but routes
 * to the in-tree zero-dependency MCP client implementation.
 */
export class McpClient {
  private server: McpServerConfig;
  private mcp: McpClientCore;
  private tools: Tool[] = [];
  ...
  constructor(mcpServer: McpServerConfig) {
    ...
    // Build the internal `McpClientConfig` from the user-facing `McpServerConfig`.
    // No initialization timeout / retry: MCP servers may need to download
    // dependencies before they speak protocol.
    const mcpClientConfig: McpClientConfig = {
      ...
      // DeskmateOAuthProvider that knows the server's oauth.* options.
      mcpServerConfig: mcpServer,
    };

    this.mcp = new McpClientCore(mcpClientConfig);
    this.lastError = null;
