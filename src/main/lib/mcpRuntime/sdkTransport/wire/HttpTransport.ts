...
import { McpAuthMetadataService, type McpAuthTokenProvider, type McpResolvedAuthMetadata } from '../../auth';
import type { McpServerConfig } from '@shared/persist/types'

export interface HttpTransportConfig {
  authService: McpAuthTokenProvider;
  serverName: string;
  url: string;
   ...
   * construct a `DeskmateOAuthProvider` that knows the server's `oauth.*`
...
/**
 * MCP Streamable HTTP / SSE transport. AbortSignal monitoring is delegated
 * to the AbortController on this instance.
 */
export class HttpTransport extends EventEmitter {
  ...
  constructor(private config: HttpTransportConfig) {
    super();
    this.emit('log', 'debug', `HttpTransport initialized for ${config.url}`);
  }
  ...
  /**
   * Pipe a response into an SSE parser.
   */
  ...
  private async _requestToken(
    ...
    return this.config.authService.getTokenForServer(this.config.serverName, metadata, {
      ...options,
