...
export class SkillManager {
  ...
  public parseSkillMarkdown(content: string): MetadataParseResult {
    try {
      ...
      logger.info({ msg: `[SkillManager] Parsed skill metadata - name: "${metadata.name}", description: "${metadata.description}"` });
  ...
  /**
   * Check whether a skill already exists in the active profile.
   */
  public async checkSkillExists(skillName: string): Promise<any | null> {
    try {
  ...
  /**
   * Install or update a skill into the active profile.
   */
  public async installSkill(
    ...
    try {
      ...
      if (fs.existsSync(skillRootDir)) {
        logger.info({ msg: `[SkillManager] Removing existing skill directory: ${skillRootDir}` });
      ...
      if (fs.existsSync(sourceDir)) {
        ...
        logger.info({ msg: `[SkillManager] Moved skill to: ${skillRootDir}` });
      } else {
        return { success: false, error: 'Source skill directory not found' };
      }

      // Save/update skill configuration to persist
      let success = true;
      try {
   ...
   */
  public createTempDirectory(prefix: string = 'deskmate-skill'): string {
    const tempDir = path.join(getTmpDir(), `${prefix}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
