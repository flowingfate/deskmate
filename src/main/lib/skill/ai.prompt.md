<!-- Last verified: 2026-07-18 (ProfileStore parameter naming) -->
# Skills System
...
| `skillInstall.ts` | `validateSkillPackage()` / `checkSkillExists(store, …)` / `installSkill(store, …)` / `linkSkill(store, …)` / 临时目录管理——所有读写显式接收 owning `ProfileStore`，替换时旧目标先移入 app tmp backup，索引写失败会回滚磁盘目标 | medium |
| `installAndActivateSkill.ts` | `installAndActivateSkill(store, …)` — 统一入口：从设备路径安装，然后按激活模式（`current-agent`、`all-agents`、`install-only` 等）应用到同一 Profile 的 agents | medium |
| `skillDeviceImporter.ts` | `addSkillFromDevice(store, …)` / `updateSkillFromDevice(store, …)` — 从 `.zip`、`.skill` 或 skill 文件夹路径复制导入新 skill，或按包自身 `SKILL.md` name 自动匹配已装 skill 并覆盖更新 | small |
...
| `importForeignAgentSkills.ts` | `importForeignAgentSkills(store, …)` — 批量导入外部 Agent skill，逐项 link/copy，main 端重读元数据并校验同名冲突 | medium |
| `skillAvailability.ts` | `getSkillAvailability(store, …)` — 检查某个 skill 是否已安装并可供指定 agent 调用 | small |
| `applySkillToAgents.ts` | `applySkillToAgents(store, …)` — 将已安装的 skill 应用到同一 Profile 的一个或多个 agent 配置 | small |
| `deleteInstalledSkill.ts` | `deleteInstalledSkill(store, …)` — UI 和内置工具共用的删除路径：移除该 Profile 的 skill 配置并从磁盘删除本地包 | small |
| `removeSkillsFromAgents.ts` | `removeSkillsFromAgents(store, …)` — 从同一 Profile 的一个或多个 agent 配置中移除 skill 名称，不卸载底层本地包 | small |
...
- **Profile 路由**：renderer `skills:*` IPC 不传 `profileId`。handler 从 sender 的绑定主窗口取得 owning `ProfileStore`，并把它显式传入所有写路径；AppCommand 同样从 `ctx.profile.store` 注入。目录浏览、原生文件选择和 overwrite 对话框同样使用 sender window。Skill 服务层不得读取 `defaultProfileId`。
