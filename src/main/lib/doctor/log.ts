...
function getLogPath(taskId: string): string {
  ...
  const logPath = path.join(projectRoot, 'tmp', 'doctor', `${taskId}.log.md`);
  fs.mkdirSync(path.dirname(logPath), { recursive: true });
  return logPath;
}

export function clearDebugLog(taskId: string): void {
  if (!isDev) return;
  try {
    fs.writeFileSync(getLogPath(taskId), `# Doctor Agent Log\n\n_Started: ${new Date().toISOString()}_\n\n`);
  } catch (err) {
    console.warn('[doctor/log] clearDebugLog failed:', err);
  }
}

export function appendDebugLog(taskId: string, section: string, content: string): void {
  if (!isDev) return;
  try {
    const timestamp = new Date().toISOString().slice(11, 23);
    const entry = `## [${timestamp}] ${section}\n\n${content}\n\n---\n\n`;
    fs.appendFileSync(getLogPath(taskId), entry);
  } catch (err) {
