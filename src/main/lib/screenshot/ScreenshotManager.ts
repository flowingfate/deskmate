...
import { createWindow, mainWindow } from '../../startup/wins';
...
import { PRELOAD_PATH } from '../buildPaths';
...
export class ScreenshotManager {
  ...
  async checkScreenCapturePermission(): Promise<boolean> {
    ...
    logger.info({ msg: `[ScreenshotManager] Screen capture permission status: ${status}` });
  ...
  async capture(callback = true): Promise<CaptureResult> {
    ...
    try {
      ...
      logger.info({ msg: `[ScreenshotManager] Starting capture for ${displays.length} display(s), isDev=${this.isDev}` });
      ...
      logger.error({ msg: `[ScreenshotManager] Failed to open: ${errorStr}` });
  ...
  private async captureAllDisplays(displays: Electron.Display[]) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      logger.info({ msg: `[ScreenshotManager] desktopCapturer returned ${sources.length} sources (attempt ${attempt}/${maxRetries})` });
      ...
      for (const display of displays) {
        ...
        if (!source || source.thumbnail.isEmpty()) {
          ...
          logger.warn({ msg: `[ScreenshotManager] Capture attempt ${attempt}/${maxRetries} failed for display ${display.id} ` +
      ...
      if (allCaptured) {
        logger.info({ msg: `[ScreenshotManager] All displays captured successfully on attempt ${attempt}` });
        return results;
      }
  ...
  private async createDisplayWindowForParallel(display: Electron.Display): Promise<WindowReadyState> {
    ...
    const windowOptions: BrowserWindowConstructorOptions = {
      ...
      webPreferences: {
        preload: PRELOAD_PATH.screenshot,
    ...
    const window = createWindow(windowOptions, { role: 'screenshot', tag: `display:${id}` });
  ...
  async saveToFile(displayId: number, rect: SelectionRect, imageData?: Buffer, savePath?: string): Promise<SaveToFileResult> {
    try {
      ...
      const defaultFilename = `deskmate-screenshot-${timestamp}.png`;
      const defaultDir = savePath || app.getPath('downloads');

      const parentWindow = this.displays.get(displayId)?.window ?? mainWindow();
      const dialogOptions = {
        defaultPath: path.join(defaultDir, defaultFilename),
        filters: [{ name: 'PNG Image', extensions: ['png'] }],
      };

      // Electron 35 overload resolution quirk
