...
import { getQuickStartImageCacheDir } from '@main/persist/lib/path';
...
class QuickStartImageCacheManager {
  ...
  private constructor() {
    this.cacheDir = getQuickStartImageCacheDir();
    this.ensureCacheDir();
  }
  ...
  private ensureCacheDir(): void {
    if (!fs.existsSync(this.cacheDir)) {
      ...
      logger.info({ msg: `[QuickStartImageCache] Created cache directory: ${this.cacheDir}` });
    }
  }
  ...
  async cacheImage(agentName: string, imageUrl: string): Promise<string | null> {
    try {
      ...
      if (!response.ok) {
        logger.error({ msg: `[QuickStartImageCache] Failed to fetch: ${fetchUrl}, status: ${response.status}` });
        return null;
      }

      const buffer = await response.arrayBuffer();
      fs.writeFileSync(cachePath, Buffer.from(buffer));
      logger.info({ msg: `[QuickStartImageCache] Cached: ${imageUrl} -> ${cachePath}` });

      return cachePath;
    } catch (error) {
      logger.error({ msg: `[QuickStartImageCache] Error caching image: ${imageUrl}`, mod: error instanceof Error ? error.message : String(error) });
      return null;
    }
  }
  ...
  async getOrCacheImage(agentName: string, imageUrl: string): Promise<string | null> {
    try {
      ...
      logger.error({ msg: `[QuickStartImageCache] Error getting/caching image: ${imageUrl}`, mod: error instanceof Error ? error.message : String(error) });
      return null;
    }
  }
  ...
  clearAgentCache(agentName: string): void {
    ...
    if (fs.existsSync(agentCacheDir)) {
      try {
        ...
        logger.info({ msg: `[QuickStartImageCache] Cleared cache for agent: ${agentName}` });
      } catch (error) {
        logger.error({ msg: `[QuickStartImageCache] Failed to clear cache for agent: ${agentName}`, mod: error instanceof Error ? error.message : String(error) });
      }
    }
  }
  ...
  clearAllCache(): void {
    if (fs.existsSync(this.cacheDir)) {
      try {
        ...
        this.ensureCacheDir();
        logger.info({ msg: '[QuickStartImageCache] Cleared all cache' });
      } catch (error) {
        logger.error({ msg: `[QuickStartImageCache] Failed to clear all cache: ${error instanceof Error ? error.message : String(error)}` });
      }
    }
  }
