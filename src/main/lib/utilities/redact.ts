 ...
 *
 * Alias-aware and schedule-JSON–aware redaction for the Download Debug Info
 * feature.
 */
...
export interface RedactorOptions {
  /** Current profile id — will be replaced with <REDACTED_ALIAS> everywhere (placeholder marker kept stable for downstream tools). */
...
export function createRedactor(options?: RedactorOptions): (input: string) => string {
  ...
  const profileIdReplacements: Array<[RegExp, string]> = [];
  if (options?.profileId) {
    const profileId = options.profileId;
    // Whole-word match for profile id in text content
    profileIdReplacements.push([
      new RegExp(`\\b${escapeRegExp(profileId)}\\b`, 'g'),
      '<REDACTED_ALIAS>',
    ]);
    // Path segment match (e.g. /profiles/<id>/ or \profiles\<id>\)
    profileIdReplacements.push([
      new RegExp(`([/\\\\])${escapeRegExp(profileId)}([/\\\\])`, 'g'),
      '$1<REDACTED_ALIAS>$2',
    ]);
  }
  ...
  return (input: string): string => {
    let out = input;
    // Profile id first — before general patterns that might partially match
    for (const [re, replacement] of profileIdReplacements) {
