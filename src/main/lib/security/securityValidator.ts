...
export class SecurityValidator {
  ...
  static validateToolPaths(
    ...
    logger.info({ msg: '[SecurityValidator] 🔐 Validating tool paths', mod: 'validateToolPaths', toolName, workspacePath, toolArgsKeys: Object.keys(toolArgs || {}) });
    ...
    logger.info({ msg: '[SecurityValidator] ✅ Tool paths validation result', mod: 'validateToolPaths', toolName, approved: validationResult.approved, pathsOutsideWorkspaceCount: validationResult.pathsOutsideWorkspace.length });
  ...
  static validateBatchToolCalls(
    ...
    logger.info({ msg: '[SecurityValidator] 🔐 Starting batch validation', mod: 'validateBatchToolCalls', toolCallsCount: toolCalls.length, toolNames: toolCalls.map(tc => tc.function.name), workspacePath });

    const validationResults: ToolCallValidationResult[] = [];

    for (const toolCall of toolCalls) {
      logger.info({ msg: '[SecurityValidator] Validating tool call', mod: 'validateBatchToolCalls', toolCallId: toolCall.id, toolName: toolCall.function.name });
      ...
      logger.info({ msg: '[SecurityValidator] Tool call validation result', mod: 'validateBatchToolCalls', toolCallId: toolCall.id, toolName: toolCall.function.name, approved: validationResult.approved, pathsOutsideWorkspaceCount: validationResult.pathsOutsideWorkspace.length, pathsOutsideWorkspace: validationResult.pathsOutsideWorkspace });
    }
    ...
    logger.info({ msg: '[SecurityValidator] ✅ Batch validation completed', mod: 'validateBatchToolCalls', allApproved: batchResult.allApproved, needsApproval: batchResult.needsApproval, validationResultsCount: batchResult.validationResults.length });
