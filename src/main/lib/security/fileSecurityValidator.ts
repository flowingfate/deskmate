...
import { getProfilesRootPath } from '@main/persist/lib/path';
...
export class FileSecurityValidator {
  ...
  private static isPathInWhitelist(normalizedFilePath: string): boolean {
    try {
      // Build the base path for the skills directory: ${appData}/profiles/
      const profilesBasePath = getProfilesRootPath();
  ...
  static extractPathsFromToolArgs(toolArgs: any): string[] {
    ...
    logger.info({ msg: '[FileSecurityValidator] 🔍 Starting path extraction from tool args', mod: 'extractPathsFromToolArgs', toolArgsKeys: Object.keys(toolArgs || {}), toolArgsPreview: JSON.stringify(toolArgs).substring(0, 200) });
    ...
    if (basePath) {
      logger.info({ msg: '[FileSecurityValidator] 📂 Found base path', mod: 'extractPathsFromToolArgs', basePath });
    }
    ...
    const extractPaths = (obj: any, key: string = '', depth: number = 0) => {
      ...
      if (typeof obj === 'string') {
        ...
        if (key === 'command' || key === 'cmd') {
          logger.info({ msg: '[FileSecurityValidator] 🔍 Parsing command for paths', mod: 'extractPathsFromToolArgs', commandPreview: obj.substring(0, 100), commandLength: obj.length });

          const extractedPaths = CommandParser.extractPathsFromCommand(obj);

          logger.info({ msg: '[FileSecurityValidator] 📊 Extracted paths from command', mod: 'extractPathsFromToolArgs', extractedPathsCount: extractedPaths.length, extractedPaths: extractedPaths, basePath: basePath });
          ...
          for (const extractedPath of extractedPaths) {
            if (basePath && !this.isAbsolutePath(extractedPath)) {
              ...
              logger.debug({ msg: '[FileSecurityValidator] Combined relative path with base', mod: 'extractPathsFromToolArgs', relativePath: extractedPath, basePath, result: combinedPath });
            } else {
              // Absolute path: use directly
              pathsToValidate.push(extractedPath);
              logger.debug({ msg: '[FileSecurityValidator] Added absolute path', mod: 'extractPathsFromToolArgs', absolutePath: extractedPath });
            }
          }
          return;
        }
    ...
    logger.info({ msg: '[FileSecurityValidator] ✅ Path extraction completed', mod: 'extractPathsFromToolArgs', totalPathsFound: pathsToValidate.length, paths: pathsToValidate });
  ...
  static validateToolPathsInWorkspace(
    ...
    logger.info({ msg: '[FileSecurityValidator] 🔐 Starting workspace validation', mod: 'validateToolPathsInWorkspace', workspacePath, pathsToValidateCount: pathsToValidate.length, pathsToValidate });

    for (const filePath of pathsToValidate) {
      const validation = this.isPathInWorkspace(filePath, workspacePath);

      logger.info({ msg: '[FileSecurityValidator] Validation result for path', mod: 'validateToolPathsInWorkspace', filePath, isInWorkspace: validation.isInWorkspace, normalizedPath: validation.normalizedPath, err: validation.error });
    ...
    logger.info({ msg: '[FileSecurityValidator] ✅ Workspace validation completed', mod: 'validateToolPathsInWorkspace', allPathsValid: result.allPathsValid, pathsOutsideWorkspaceCount: result.pathsOutsideWorkspace.length, pathsOutsideWorkspace: result.pathsOutsideWorkspace });
