...
export default function(ctx: Context) {
   ...
   * helper function
   */
  async function responseStream(wc: Electron.WebContents, chunk_stream: Stream<StreamingChunk>) {
    const sender = mainToRender.bindWebContents(wc);
    for await (const chunk of chunk_stream) {
  ...
  async function runOrchestrator(
    ...
    const agent = Agent.getOrCreate(profileId, agentId);
  ...
  async function stopNewOrchestratorSession(agentId: string, chatSessionId: string): Promise<void> {
    const session = Agent.get(agentId)?.sessions.get(chatSessionId);
  ...
  handle.canEditUserMessage(async (_event, agentId, chatSessionId, messageId) => {
    try {
      if (!agentId || !chatSessionId) {
        return { success: false, error: 'agentId and chatSessionId are required' };
      }
  ...
  handle.cancelChatSession(async (_event, agentId, chatSessionId, msgTrace) => {
    // cancel 是个独立的主链路 span：从 renderer 接力 trace 时挂在 in-flight chat.send
    ...
    try {
      await stopNewOrchestratorSession(agentId, chatSessionId);
  ...
  handle.removeAgentInstance(async (_event, agentId, chatSessionId) => {
    try {
      Agent.get(agentId)?.removeSession(chatSessionId);
      return { success: true };
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  ...
  handle.forkChatSession(async (_event, agentId, sourceChatSessionId) => {
    try {
  ...
  handle.importChatSession(async (_event, agentId) => {
    try {
  ...
  handle.loadChatSessionSnapshot(async (_event, agentId, chatSessionId) => {
    try {
      if (!agentId || !chatSessionId) {
        return { success: false, error: 'agentId and chatSessionId are required' };
      }
      ...
      const liveSession = Agent.get(agentId)?.sessions.get(chatSessionId);
  ...
  handle.loadJobRunSnapshot(async (_event, agentId, jobId, runId) => {
    try {
  ...
  handle.markSessionRead(async (_event, agentId, chatSessionId) => {
    try {
      if (!agentId || !chatSessionId) {
        return { success: false, error: 'agentId and chatSessionId are required' };
      }
  ...
  handle.markJobRunRead(async (_event, agentId, jobId, runId) => {
    try {
      if (!agentId || !jobId || !runId) {
        return { success: false, error: 'agentId, jobId and runId are required' };
      }
