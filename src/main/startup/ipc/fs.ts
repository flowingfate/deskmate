...
export default function(ctx: Context) {
  ...
  handle.deletePaths(async (_event, paths) => {
    try {
  ...
  // File system handlers for MCP config import
  handle.exists(async (_event, filePath) => {
    try {
  ...
  handle.listDir(async (_event, dirPath) => {
    try {
  ...
  handle.access(async (_event, filePath) => {
    try {
  ...
  handle.readFile(async (_event, filePath, encoding?) => {
    try {
  ...
  handle.writeFile(async (
    ...
    try {
      ...
      const strategy: ImportConflictResolution = options?.conflictResolution || 'reject';
  ...
  handle.stat(async (_event, filePath) => {
    try {
      const stats = fs.statSync(filePath);
  ...
  handle.expandPath(async (_event, filePath) => {
    try {
  ...
  handle.getFileMetadata(async (_event, filePath) => {
    try {
      const stats = fs.statSync(filePath);
  ...
  handle.downloadFile(async (_event, url, destPath) => {
    try {
      ...
      log.info({ msg: `[fs:downloadFile] Downloaded ${url} to ${destPath}`, mod: 'fs:downloadFile' });
      ...
      log.error({ msg: `[fs:downloadFile] Failed to download ${url}:`, mod: 'fs:downloadFile', err: error });
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error'
      };
  ...
  handle.selectFiles(async (_event, options?) => {
    ...
    try {
      ...
      const result = await dialog.showOpenDialog(win, dialogOptions);
      logger.info({ msg: '[fs:selectFiles] Dialog closed', mod: 'fs:selectFiles', raw: JSON.stringify(result) });

      // Handle the result properly - check if it's the old format or new format
      if (Array.isArray(result)) {
        // Old API format (just file paths array)
        if (result.length === 0) {
          logger.info({ msg: '[fs:selectFiles] Canceled (old API format)', mod: 'fs:selectFiles' });
          return {
            success: false,
            error: 'File selection canceled'
          };
        }
        logger.info({ msg: '[fs:selectFiles] Selected (old API format)', mod: 'fs:selectFiles', filePaths: result });
        return {
          success: true,
          filePaths: result
        };
        ...
        const dialogResult = result as any;
        if (dialogResult.canceled || !dialogResult.filePaths || dialogResult.filePaths.length === 0) {
          logger.info({ msg: '[fs:selectFiles] Canceled', mod: 'fs:selectFiles' });
          return {
            success: false,
            error: 'File selection canceled'
          };
        }
        logger.info({ msg: '[fs:selectFiles] Selected', mod: 'fs:selectFiles', filePaths: dialogResult.filePaths });
        return {
          success: true,
      ...
      logger.error({ msg: '[fs:selectFiles] Error', mod: 'fs:selectFiles', err: error });
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error'
      };
