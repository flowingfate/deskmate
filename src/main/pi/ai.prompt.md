<!-- Last verified: 2026-07-16 (新增 Agent 委派运行时共享契约边界，尚未生产接线) -->
...
| `subagent/` | **建设中**的 Agent 委派运行时边界。当前只有 shared request/result/state 与 normalization；manager/session/persist/tool 尚未接线，旧 `lib/subAgent` 仍是生产路径。新代码禁止依赖旧实现。详见 [`subagent/ai.prompt.md`](./subagent/ai.prompt.md) | 小 |
