...
export class McpConfigLlmFormatter {
  // System prompt instructing the LLM to normalize varied MCP config inputs into the canonical Deskmate format.
...
1. **Format Detection and Conversion**: Automatically identify variants of MCP server configuration (settings.json-style, mcp.json-style, etc.) and convert to the standard format
...
1. **Format Detection**: Identify the input shape (settings.json-style, mcp.json-style, plain server-config object).
...
{
  "success": true,
  "originalFormat": "settings.json (mcp.servers)",
  "transportType": "stdio",
...
#### 1. Settings JSON Format (mcp.servers)
\`\`\`json
{
  "mcp": {
    "servers": {
...
Do NOT nest the configuration under the server name. The config field should directly contain the server configuration object.`;
   ...
   */
  static async formatMcpConfig(userInputMcpConfig: string, profileId: string): Promise<McpConfigFormatterResponse> {
    try {
...
${userInputMcpConfig}`;
      ...
      const rawResponse = await runUtilityCompletion({
        modelKey: 'github-copilot::claude-haiku-4.5',
        profileId,
        systemPrompt: this.SYSTEM_PROMPT,
        userPrompt: llmParams.prompt,
