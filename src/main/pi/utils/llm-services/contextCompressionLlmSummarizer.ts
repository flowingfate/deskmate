...
export interface ContextCompressionLlmSummarizerParams {
  ...
  maxRetries?: number;
  /**
   * 主链路 tracer（来自 chat.compress span）。提供则每次 attempt 写一对
   * `chat.compress.summary` start/ok/failed 行；缺省时不写主链路 log
   * （sub-agent / 命令行 / 测试均可省略）。
   */
  tracer?: Tracer;
}
...
Please generate a structured summary according to the above requirements:`;
  ...
  static async summarize(
    ...
    // 缺省 Tracer.noop —— 仍写 chat.compress.summary 日志，只是没 tid/sid，
    // 让"按 mod 看全集"和"按 tid 追主链路"两条路径都成立。
    const parent = params.tracer ?? Tracer.noop;
    let lastError: Error | null = null;

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      // 每个 attempt 一个 child span：start/ok|failed 共享 sid，dur 表达本次 attempt 耗时。
      const tracer = parent.derive().bind({
        mod: 'chat.compress.summary',
        model: this.MODEL,
        attempt,
        maxRetries,
      });

      log.info(tracer.fields({ msg: 'llm start', promptLen: prompt.length }));

      try {
        const response = await runUtilityCompletion({
          modelKey: `github-copilot::${this.MODEL}`,
        ...
        log.info(tracer.fields({ msg: 'llm ok', summaryLen: summary.length }, 'self'));

        return { success: true, summary, attempts: attempt };
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(String(error));

        log.warn(tracer.fields({ msg: 'llm failed', err: lastError }, 'self'));
