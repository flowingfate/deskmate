<!-- Last verified: 2026-07-16 (Step 5：delegate-only AsyncLocalStorage context 已完成) -->
# pi/subagent 模块 — Agent 委派运行时

> 普通 Agent 在父 session 中被委派执行一次任务的运行时边界。Sub-Agent 是运行角色，不是第二种配置实体。
> 当前已固定共享契约、未注册的顶层 cmdline facade、delegate-only execution context 与 capability boundary；manager、session、持久化尚未生产接线。
...
以下文件是后续步骤的规划边界，**当前尚未实现**：`prompt.ts`、`submitResult.ts`、`subrunStore.ts`（或 persist adapter）、`session.ts`、`manager.ts`。
...
- 依赖 delegate-only context 约束 delegated capability；后续再负责显式结果提交、subrun 生命周期和 orchestration；
...
- catalog seam：delegate context 内 `buildToolCatalogForAgent` 自动过滤 LocalTool；Step 7 再实现 submit_result 的最小私有 route。
