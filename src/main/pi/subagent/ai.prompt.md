<!-- Last verified: 2026-07-16 (Step 9：manager 按 Profile 绑定；顶层 subagent 与旧后端 cutover 已完成) -->
...
| `manager.ts` | `SubAgentManager.forProfile(profile)` 返回唯一 profile-bound manager：授权、reservation、stale recovery、limits、timeout/cancel、active state 与 state subscription | 大 |
