<!-- Last verified: 2026-07-16 (Step 8：BaseSession 驱动的单个 delegated run 已落盘，尚未生产接线) -->
# pi/subagent 模块 — Agent 委派运行时

> 普通 Agent 在父 session 中被委派执行一次任务的运行时边界。Sub-Agent 是运行角色，不是第二种配置实体。
> 当前已固定共享契约、未注册的顶层 cmdline facade、delegate-only execution context、parent-scoped subrun persist store、正式结果提交，以及可执行单个 persisted run；manager/production 注册仍未接线。
...
| `prompt.ts` | 执行 Agent 的通用 identity/knowledge/skills/global prompt + delegated run contract；通用 prompt 本身不含任何委派指导 | 小 |
...
以下文件仍是后续步骤的规划边界，**当前尚未实现**：`manager.ts`。
...
- 依赖 delegate-only context 约束 delegated capability；负责显示提交、正式结果归并与单 run 生命周期，Step 9 再接 manager orchestration；
...
- session seam：`SubAgentSession({ subrun, signal, parentTracer?, callbacks? })`，`run()` 返回 `{ kind:'result', result } | { kind:'not_pending', status }`。它在最外层建立 delegate scope，使用执行 Agent config/catalog/prompt，局部收集 usage/deliverables；每次调用 BaseSession 都是完整、自然结束的 ReAct user turn，`submit_result` 只由 controller 被动记录，绝不截断 loop。未提交时外层才 append/flush 一条真实 reminder user message 并创建下一完整 turn；`maxTurns` 跨 turn 累计，随后严格按 formal result → terminal `Subrun.finish` → callback/return 完成；
...
- `SubAgentSession` 不读取 parent history；`parent_summary` 只以明确“不可信参考”提示包入 delegated prompt。它不注册工具、不开 timer、不判授权；这些属于 Step 9 manager。
