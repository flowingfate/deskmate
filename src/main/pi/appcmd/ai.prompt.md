<!-- Last verified: 2026-07-16 (AppCmdContext 改为 agent/delegate mode union) -->
...
| `makeRouterCommand.ts` | 顶层 router 核心。`makeRouterCommand({ name, synopsis, registry, helpFooter? })` 把 registry 包成 `AppCommand`；动态 help/description 枚举成员，`helpFooter` 只为顶层 help 追加领域规则。`app` / `web` 及未注册的新 `subagent` facade 共用同一实现 | 小 |
...
### AppCmdContext = ToolContext execution union + stdio helpers

```ts
type AppCmdContext =
  | { mode: 'agent'; agentId: string; sessionId: string; /* common + stdio */ }
  | { mode: 'delegate'; agentId: string; sessionId: string; delegateId: string; /* common + stdio */ };
```

`agentId/sessionId` 固定 parent session；delegateId 是 execution Agent。dispatcher 逐字段构造两个分支，不 spread ToolContext；delegate mode 同时取代旧 `isSubAgent` 递归标记。`web download` 复用 `toResolveContext(ctx)`，完整保留 discriminant。
