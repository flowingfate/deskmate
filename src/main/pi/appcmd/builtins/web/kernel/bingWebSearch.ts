/**
 * `app web search <query...>` 内核 —— Playwright headless Chromium 驱动的 Bing 网页搜索。
 *
 * 历史:由 `pi/tools/impl/bingWebSearch.ts` 平移而来,**body 一字不改**,
 * 只把 `WebSearchToolArgs/Result/Item` shared 类型 inline 到本文件 ——
 * shared 形态的唯一非内核消费者(`WebSearchToolCallView.tsx`)已删除,
 * kernel 不再有 cross-process 边界。
 *
 * 行为契约:
 * - 接受 1-10 个 query,parallel 浏览器实例(每个 query 一个 context),
 *   `lang`/`locale` 必填(`en/us` 或 `zh/cn`)以触发 Bing 的语言/区域路由。
 * - 反检测:devices 指纹随机化 + cookie state 持久化(`tmpdir/deskmate-bing-
 *   browser-state.json`)+ navigator 属性覆盖 + WebGL 指纹随机化。
 * - 失败结果:单 query 失败回 errors 数组,**不**抛 —— 多 query 场景下
 *   一个失败不应让整批塌掉。
 * - `options.signal` 通过 page.close() 强制中断:外部 abort 时关闭 page
 *   导致 page.goto() throw,链式收敛成 "Bing search aborted"。
 */

import { PlaywrightManager } from '@main/lib/playwright';
...
// ============ Type definitions(原 `@shared/types/toolCallArgs/webSearch.ts` 平移)============
...
export class BingWebSearchTool {
    ...
    try {
      ...
      for (let i = 0; i < checks; i++) {
        ...
        if (currentUrl !== previousUrl) {
          logger.debug({ msg: `[BingWebSearchTool] Page URL changed: ${previousUrl} → ${currentUrl}` });
          return false;
        }
      ...
      logger.debug({ msg: `[BingWebSearchTool] Page stability verified: ${previousUrl}` });
      return true;
    } catch (error) {
      logger.warn({ msg: `[BingWebSearchTool] Page stability check failed: ${String(error)}` });
      return false;
    }
  }
  ...
  private static parseBingSearchResults(html: string, query: string, maxResults: number = 5): BingSearchResult[] {
    ...
    try {
      ...
      logger.debug({ msg: `[BingWebSearchTool] Found ${algoMatches.length} search result containers` });
      ...
      for (let i = 0; i < algoMatches.length && results.length < maxResults; i++) {
        try {
          ...
          results.push(result);
          logger.debug({ msg: `[BingWebSearchTool] Parsing result # ${result.index} : "${result.title}"` });

        } catch (error) {
          logger.warn({ msg: `[BingWebSearchTool] Parsing result # ${i + 1} :`, mod: String(error) });
        }
      }
      ...
      logger.error({ msg: `[BingWebSearchTool] Failed to parse Bing search results: ${String(error)}` });
      return [];
    }
  }
  ...
  static async execute(args: BingWebSearchToolArgs, options?: { signal?: AbortSignal }): Promise<BingWebSearchToolResult> {
    try {
      ...
      logger.debug({ msg: '[BingWebSearchTool] Checking Playwright Chromium browser...' });
      const browserCheck = await PlaywrightManager.getInstance().ensureBrowserInstalled();
      if (!browserCheck.installed) {
        logger.error({ msg: '[BingWebSearchTool] Playwright Chromium browser not installed and auto-install failed' });
        return {
          success: false,
      ...
      logger.debug({ msg: `[BingWebSearchTool] Browser check passed${browserCheck.browserPath ? ': ' + browserCheck.browserPath : ''}` });

      const allResults: BingSearchResult[] = [];
      const errors: string[] = [];

      // State file configuration
      const stateFile = path.join(os.tmpdir(), 'deskmate-bing-browser-state.json');
      ...
      if (fs.existsSync(stateFile)) {
        logger.debug({ msg: '[BingWebSearchTool] Browser state file found' });
        ...
        try {
          ...
          logger.debug({ msg: '[BingWebSearchTool] Browser state file validated' });
        } catch (e) {
          logger.warn({ msg: `[BingWebSearchTool] Browser state file is corrupted, deleting and recreating: ${String(e)}` });
          try {
            fs.unlinkSync(stateFile);
            logger.debug({ msg: '[BingWebSearchTool] Corrupted state file deleted' });
          } catch (deleteError) {
            logger.warn({ msg: `[BingWebSearchTool] Unable to delete corrupted state file: ${String(deleteError)}` });
          }
        ...
        if (fs.existsSync(fingerprintFile)) {
          try {
            ...
            logger.debug({ msg: '[BingWebSearchTool] Loaded saved browser fingerprint configuration' });
          } catch (e) {
            logger.warn({ msg: '[BingWebSearchTool] Unable to load fingerprint config file, will create new fingerprint' });
            // If the fingerprint file is also corrupted, delete it
            try {
              fs.unlinkSync(fingerprintFile);
              logger.debug({ msg: '[BingWebSearchTool] Corrupted fingerprint file deleted' });
            } catch (deleteError) {
        ...
        logger.debug({ msg: '[BingWebSearchTool] No browser state file found, creating new browser session' });
      }
      ...
      if (signal) {
        ...
        abortHandler = () => {
          logger.debug({ msg: '[BingWebSearchTool] External abort signal received' });
        };
      ...
      const searchPromises = args.queries.map(async (query, queryIndex) => {
        ...
        try {
          ...
          logger.error({ msg: `[BingWebSearchTool] ${errorMsg}` });
      ...
      logger.error({ msg: `[BingWebSearchTool] Search execution failed: ${String(error)}` });
      return {
        success: false,
        totalQueries: args.queries.length,
        totalResults: 0,
  ...
  private static async performSingleSearch(
    ...
    const performSearchAndGetHtml = async (): Promise<string> => {
      logger.debug({ msg: '[BingWebSearchTool] Starting browser (headless mode)...' });
      ...
      try {
        ...
        logger.debug({ msg: `[BingWebSearchTool] Using device configuration: ${deviceName}` });
        ...
        if (savedState.fingerprint) {
          ...
          logger.debug({ msg: '[BingWebSearchTool] Using saved browser fingerprint configuration' });
        } else {
          ...
          logger.debug({ msg: `[BingWebSearchTool] Generated new browser fingerprint configuration: ${hostConfig.locale}, ${hostConfig.timezoneId}` });
        }
        ...
        try {
          ...
          logger.debug({ msg: `[BingWebSearchTool] Navigating to Bing search page: ${searchUrl}` });
          ...
          logger.debug({ msg: '[BingWebSearchTool] Waiting for search results page to load...' });
          ...
          try {
            ...
            logger.debug({ msg: '[BingWebSearchTool] Search results appeared' });
          } catch (selectorError) {
            logger.warn({ msg: '[BingWebSearchTool] Timed out waiting for search results selector, trying to continue...' });
          }
          await page.waitForLoadState('domcontentloaded', { timeout });

          // Get current page URL
          const finalUrl = page.url();
          logger.debug({ msg: `[BingWebSearchTool] Search results page loaded: ${finalUrl}` });

          // Wait for page to stabilize
          logger.debug({ msg: '[BingWebSearchTool] Waiting for page to stabilize...' });
          ...
          try {
            ...
            logger.warn({ msg: '[BingWebSearchTool] Timed out waiting for page load state, continuing...' });
          }

          // Page stability check
          logger.debug({ msg: '[BingWebSearchTool] Checking page stability...' });
          const isStable = await this.isPageStable(page);
          if (!isStable) {
            logger.warn({ msg: '[BingWebSearchTool] Page is still navigating, waiting longer...' });
            await page.waitForTimeout(2000);
            try {
              await page.waitForLoadState('domcontentloaded', { timeout: Math.min(timeout, 15000) });
            } catch (loadError) {
              logger.warn({ msg: '[BingWebSearchTool] Retry wait for page load timed out, continuing...' });
            }

            const isStableRetry = await this.isPageStable(page);
            if (!isStableRetry) {
              logger.warn({ msg: '[BingWebSearchTool] Page did not fully stabilize, but continuing to parse results' });
            }
          }
          ...
          logger.debug({ msg: `[BingWebSearchTool] HTML content stats: original length ${fullHtml.length}, cleaned length ${html.length}` });

          // Save browser state
          try {
            logger.debug({ msg: '[BingWebSearchTool] Saving browser state...' });
            ...
            logger.debug({ msg: `[BingWebSearchTool] Browser state saved: ${stateFile}` });

            // Save fingerprint configuration
            fs.writeFileSync(fingerprintFile, JSON.stringify(savedState, null, 2), 'utf8');
            logger.debug({ msg: `[BingWebSearchTool] Fingerprint configuration saved: ${fingerprintFile}` });
          } catch (stateError) {
            logger.warn({ msg: `[BingWebSearchTool] Failed to save browser state: ${String(stateError)}` });
          }
          ...
          logger.error({ msg: `[BingWebSearchTool] Error occurred during search: ${String(error)}` });

          // Attempt to save state even if an error occurred
          try {
            logger.debug({ msg: '[BingWebSearchTool] Attempting to save browser state...' });
            const stateDir = path.dirname(stateFile);
            if (!fs.existsSync(stateDir)) {
              fs.mkdirSync(stateDir, { recursive: true });
            }
            await context.storageState({ path: stateFile });
            fs.writeFileSync(fingerprintFile, JSON.stringify(savedState, null, 2), 'utf8');
            logger.debug({ msg: '[BingWebSearchTool] Browser state saved' });
          } catch (stateError) {
            logger.warn({ msg: `[BingWebSearchTool] Failed to save browser state: ${String(stateError)}` });
          }
