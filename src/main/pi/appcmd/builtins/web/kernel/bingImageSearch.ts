...
export class BingImageSearchTool {
    ...
    try {
      ...
      for (let i = 0; i < checks; i++) {
        ...
        if (currentUrl !== previousUrl) {
          logger.debug({ msg: `[BingImageSearchTool] Page URL changed: ${previousUrl} → ${currentUrl}` });
          return false;
        }
      ...
      logger.debug({ msg: `[BingImageSearchTool] Page stability verified: ${previousUrl}` });
      return true;
    } catch (error) {
      logger.warn({ msg: `[BingImageSearchTool] Page stability check failed: ${String(error)}` });
      return false;
    }
  }
  ...
  private static parseBingImageSearchResults(html: string, query: string, maxResults: number = 5): BingImageSearchResult[] {
    ...
    try {
      ...
      logger.debug({ msg: `[BingImageSearchTool] Found ${iuscMatches.length} image search result containers` });
      ...
      for (let i = 0; i < iuscMatches.length && results.length < maxResults; i++) {
        try {
          ...
          results.push(result);
          logger.debug({ msg: `[BingImageSearchTool] Parsing result # ${result.index} : "${result.title}"` });

        } catch (error) {
          logger.warn({ msg: `[BingImageSearchTool] Parsing result # ${i + 1} :`, mod: String(error) });
        }
      }
      ...
      logger.error({ msg: `[BingImageSearchTool] Failed to parse Bing image search results: ${String(error)}` });
      return [];
    }
  }
  ...
  static async execute(args: BingImageSearchToolArgs, options?: { signal?: AbortSignal }): Promise<BingImageSearchToolResult> {
    try {
      ...
      logger.debug({ msg: '[BingImageSearchTool] Checking Playwright Chromium browser...' });
      const browserCheck = await PlaywrightManager.getInstance().ensureBrowserInstalled();
      if (!browserCheck.installed) {
        logger.error({ msg: '[BingImageSearchTool] Playwright Chromium browser not installed and auto-install failed' });
        return {
          success: false,
      ...
      logger.debug({ msg: `[BingImageSearchTool] Browser check passed${browserCheck.browserPath ? ': ' + browserCheck.browserPath : ''}` });

      const allResults: BingImageSearchResult[] = [];
      const errors: string[] = [];

      // State file configuration
      const stateFile = path.join(os.tmpdir(), 'deskmate-bing-image-browser-state.json');
      ...
      if (fs.existsSync(stateFile)) {
        logger.debug({ msg: '[BingImageSearchTool] Browser state file found' });
        ...
        try {
          ...
          logger.debug({ msg: '[BingImageSearchTool] Browser state file validated' });
        } catch (e) {
          logger.warn({ msg: `[BingImageSearchTool] Browser state file is corrupted, deleting and recreating: ${String(e)}` });
          try {
            fs.unlinkSync(stateFile);
            logger.debug({ msg: '[BingImageSearchTool] Corrupted state file deleted' });
          } catch (deleteError) {
            logger.warn({ msg: `[BingImageSearchTool] Unable to delete corrupted state file: ${String(deleteError)}` });
          }
        ...
        if (fs.existsSync(fingerprintFile)) {
          try {
            ...
            logger.debug({ msg: '[BingImageSearchTool] Loaded saved browser fingerprint configuration' });
          } catch (e) {
            logger.warn({ msg: '[BingImageSearchTool] Unable to load fingerprint config file, will create new fingerprint' });
            // If the fingerprint file is also corrupted, delete it
            try {
              fs.unlinkSync(fingerprintFile);
              logger.debug({ msg: '[BingImageSearchTool] Corrupted fingerprint file deleted' });
            } catch (deleteError) {
        ...
        logger.debug({ msg: '[BingImageSearchTool] No browser state file found, creating new browser session' });
      }
      ...
      if (signal) {
        ...
        abortHandler = () => {
          logger.debug({ msg: '[BingImageSearchTool] External abort signal received' });
        };
      ...
      const searchPromises = args.queries.map(async (query, queryIndex) => {
        ...
        try {
          ...
          logger.error({ msg: `[BingImageSearchTool] ${errorMsg}` });
      ...
      logger.error({ msg: `[BingImageSearchTool] Search execution failed: ${String(error)}` });
      return {
        success: false,
        totalQueries: args.queries.length,
        totalResults: 0,
  ...
  private static async performSingleImageSearch(
    ...
    const performSearchAndGetHtml = async (): Promise<string> => {
      logger.debug({ msg: '[BingImageSearchTool] Starting browser (headless mode)...' });
      ...
      try {
        ...
        logger.debug({ msg: `[BingImageSearchTool] Using device configuration: ${deviceName}` });
        ...
        if (savedState.fingerprint) {
          ...
          logger.debug({ msg: '[BingImageSearchTool] Using saved browser fingerprint configuration' });
        } else {
          ...
          logger.debug({ msg: `[BingImageSearchTool] Generated new browser fingerprint configuration: ${hostConfig.locale}, ${hostConfig.timezoneId}` });
        }
        ...
        try {
          ...
          logger.debug({ msg: `[BingImageSearchTool] Navigating to Bing image search page: ${searchUrl}` });
          ...
          logger.debug({ msg: '[BingImageSearchTool] Waiting for image search results page to load...' });
          ...
          try {
            ...
            logger.debug({ msg: '[BingImageSearchTool] Image result container appeared' });
          } catch {
            logger.warn({ msg: '[BingImageSearchTool] Standard image result container not found, continuing...' });
          }

          // Get current page URL
          const finalUrl = page.url();
          logger.debug({ msg: `[BingImageSearchTool] Image search results page loaded: ${finalUrl}` });

          // Wait for page to stabilize
          logger.debug({ msg: '[BingImageSearchTool] Waiting for page to stabilize...' });
          await page.waitForTimeout(1000);

          // Page stability detection - using more lenient detection, no longer relying on networkidle
          logger.debug({ msg: '[BingImageSearchTool] Checking page stability...' });
          const isStable = await this.isPageStable(page);
          if (!isStable) {
            logger.warn({ msg: '[BingImageSearchTool] Page is still navigating, waiting longer...' });
            await page.waitForTimeout(2000);

            const isStableRetry = await this.isPageStable(page);
            if (!isStableRetry) {
              logger.warn({ msg: '[BingImageSearchTool] Page did not fully stabilize, but continuing to avoid timeout' });
            }
          }
          ...
          logger.debug({ msg: `[BingImageSearchTool] HTML content stats: full length ${fullHtml.length}` });

          // Save browser state
          try {
            logger.debug({ msg: '[BingImageSearchTool] Saving browser state...' });
            ...
            logger.debug({ msg: `[BingImageSearchTool] Browser state saved: ${stateFile}` });

            // Save fingerprint configuration
            fs.writeFileSync(fingerprintFile, JSON.stringify(savedState, null, 2), 'utf8');
            logger.debug({ msg: `[BingImageSearchTool] Fingerprint configuration saved: ${fingerprintFile}` });
          } catch (stateError) {
            logger.warn({ msg: `[BingImageSearchTool] Failed to save browser state: ${String(stateError)}` });
          }
          ...
          logger.error({ msg: `[BingImageSearchTool] Error occurred during search: ${String(error)}` });

          // Attempt to save state even if an error occurred
          try {
            logger.debug({ msg: '[BingImageSearchTool] Attempting to save browser state...' });
            const stateDir = path.dirname(stateFile);
            if (!fs.existsSync(stateDir)) {
              fs.mkdirSync(stateDir, { recursive: true });
            }
            await context.storageState({ path: stateFile });
            fs.writeFileSync(fingerprintFile, JSON.stringify(savedState, null, 2), 'utf8');
            logger.debug({ msg: '[BingImageSearchTool] Browser state saved' });
          } catch (stateError) {
            logger.warn({ msg: `[BingImageSearchTool] Failed to save browser state: ${String(stateError)}` });
          }
