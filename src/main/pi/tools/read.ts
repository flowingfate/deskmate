...
const DESCRIPTION = [
  ...
  '',
  '# HTML query (`.html` / `.htm` only — append `?<query>` to path)',
  'HTML reads are structure-first and NEVER dump raw HTML. Bare `page.html` returns a DOM outline. The `?query` axis is orthogonal to `:<sel>`.',
  '- `page.html` — DOM outline (tags / ids / classes + suggested selectors)',
  '- `page.html?mode=section&section=main` — clean text of a semantic block (main | article | body | head; default body)',
  '- `page.html?mode=selector&selector=%23content` — clean text matching a CSS selector (`#id` / `.class` / `tag`; URL-encode `#` as `%23`)',
  '- `page.html:raw` / `page.html:50-100` — escape hatch: read the HTML as plain text (verbatim / line range). For online pages use `web fetch` instead.',
  '',
  '<critical>',
