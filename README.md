...
| 原生 / 功能 | `sharp`、`@vscode/ripgrep`、`playwright-core`、`node-screenshots`、`selection-hook` |
...
│   ├── preload/                     主窗口 / toolbar / screenshot / log-viewer 各自独立 preload
│   ├── renderer/                    React SPA（3 个 BrowserWindow 共享代码池）
...
- **渲染器（`src/renderer/`）** — 三个独立 `BrowserWindow`：主窗口、截图、工具栏；dev 还有 Log Viewer。React 18 + atom 状态库。
