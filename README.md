<div align="center">
  <img src="dark.svg" width="120" alt="Deskmate AI Studio logo" />

# Deskmate AI Studio

桌面 AI 助手 · 可定制 Agent · MCP 工具 · 长期记忆 · 子 Agent 并行

</div>

Deskmate AI Studio 是一款基于 Electron 的桌面 AI 助手：创建、配置并与可定制的 AI Agent 对话。Agent 能通过 Model Context Protocol（MCP）调用外部工具，用本地工具与 `app` 伪 shell 操作文件系统、终端与浏览器，按 cron / 一次性计划运行任务，并可 spawn 子 Agent 执行有界并行工作。

<div align="center">
  <img src="docs/imgs/home.png" width="880" alt="Deskmate 首屏" />
</div>

> 本项目最初 fork 自 [Open-Kosmos](https://github.com/microsoft/open-kosmos)，目前仅复用极小部分代码。

> ⚠️ **早期开发阶段**：架构与数据结构尚不稳定，短期内会有破坏性更新——可能不兼容历史数据、需清理本地数据或重置配置。请勿在生产环境依赖本应用，也不要在其中保存无备份的重要数据。

> **代码权威文档在 [`ai.prompt/`](ai.prompt/) 与各模块目录下的 `ai.prompt.md`。** 本 README 只面向"快速上手 + 概览"。
...
| 主进程语言 | TypeScript 6.x，Node ≥ 20.19 |
...
- **Node.js** ≥ 20.19（`package.json` 的 `engines.node` 为 `>=20.19.0`）
- **npm**
...
│   │   ├── lib/                     audio / mcp / userData / scheduler ...
